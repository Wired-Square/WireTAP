// ui/src/utils/dashboardLayouts.ts
//
...
const DASHBOARD_LAYOUTS_KEY = 'graph.layouts';
...
export async function getAllDashboardLayouts(): Promise<DashboardLayout[]> {
  return (await storeGet<DashboardLayout[]>(DASHBOARD_LAYOUTS_KEY)) || [];
}
...
export async function saveDashboardLayout(
  ...
  await storeSet(DASHBOARD_LAYOUTS_KEY, layouts);
  return newLayout;
}
...
export async function updateDashboardLayout(
  id: string,
  updates: Partial<Pick<DashboardLayout, 'name'>>,
): Promise<boolean> {
  const layouts = await getAllDashboardLayouts();
