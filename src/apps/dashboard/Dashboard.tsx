...
function DashboardInner() {
  ...
  const setIoProfile = useDashboardStore((s) => s.setIoProfile);
  ...
  const loadSavedLayouts = useDashboardStore((s) => s.loadSavedLayouts);
  ...
  const handleToggleRawView = useCallback(() => {
    if (!rawViewMode) {
      ...
      const { panels, layout } = useDashboardStore.getState();
  ...
  useEffect(() => {
    ...
    if (preferredCatalogs.length === 1) {
      ...
      tlog.debug(`[Dashboard] preferred decoder → ${path}`);
      useSessionStore.getState().setSessionCatalogPath(sessionId, path);
    } else if (preferredCatalogs.length > 1) {
      tlog.debug(`[Dashboard] multiple preferred decoders: ${preferredCatalogs.join(", ")}`);
      ...
      tlog.debug(`[Dashboard] carrying over local decoder to session → ${catalogPath}`);
  ...
  const ioPickerProps = useIOSourcePickerHandlers({
    ...
    mergeOptions: (options) => mergeSerialConfig(useDashboardStore.getState().serialConfig, options),
  });
...
export default withFrameIdFormat(DashboardInner);
