...
- **Modbus → Registers: split aggregated frames into individual registers**: A Modbus `[frame.modbus.*]` frame bundles several signals into one polled read, so the Registers table showed one row per frame (e.g. `ems_control (13049)` as a single "3 signals" row), hiding that `EMS_Mode`, `Forced_Charge_Discharge_Cmd` and `Forced_Charge_Discharge_Power` actually live at registers 13049/13050/13051. A new **split toggle** in the Registers-tab header (a group/ungroup icon beside the hex/ASCII and time switches) breaks each frame into a group header plus **one row per signal at its real register number** (`base + ⌊start_bit / 16⌋`), with an accent left-border tying each group together; every split row shows that signal's own raw byte-slice (still honouring the hex/ASCII toggle) and its decoded value. The frame picker mirrors this — with split on, each frame gains a muted second line listing its constituent `register name`s. Polls stay per-frame and picker selection stays frame-level, so this is display-only and polling is unaffected; the choice is session-local, mirroring the existing toggles. Backed by a shared [`signalRegister`/`signalByteRange`](src/utils/modbusRegisters.ts) helper, with the frame's TOML name threaded through for the group header. [src/apps/modbus/views/ModbusRegistersView.tsx](src/apps/modbus/views/ModbusRegistersView.tsx), [src/apps/modbus/Modbus.tsx](src/apps/modbus/Modbus.tsx), [src/components/FramePicker.tsx](src/components/FramePicker.tsx).
