...
- **Canonical catalogue library + decode-in-Rust streaming (backend)**: The shared [`wiretap-catalog`](https://github.com/Wired-Square/wiretap-lib-rs) crate (v0.2.0) has grown from Modbus-only into the single source of truth for **reading, validating, decoding and writing** catalogues across **CAN, Serial and Modbus** — the CAN/Serial parser (mirror/copy inheritance, header-field masks), the signal decode engine (factor/offset, byte/word order, `enum`/`hex`/`ascii`/`utf8`/`unix_time`, mux ranges/lists/nesting — ported from the frontend's `bits.ts`/`signalDecode.ts`/`muxCaseMatch.ts` and single-sourced with the Modbus decoder), validation, and DBC import/export all now live in the crate. WireTAP exposes it over the existing binary WebSocket: a `catalog.*` request/response surface (`catalog.parse` / `validate` / `import_dbc` / `export_dbc`) and, crucially, a **decode-on-attach push stream** — `catalog.attach { session_id, content }` binds a catalogue to a session, after which `send_new_frames` decodes each batch **once, in Rust** and pushes a new `DecodedSignals` (0x14) message alongside the raw `FrameData`, so the frontend stops re-decoding every frame on the UI thread. Raw frames keep flowing for Discovery/Analysis/raw-hex/Calculator; attachments auto-detach on unsubscribe. The crate (now v0.3.0) also resolves `frame_id_mask` and extracts header fields (CAN id / serial bytes) in the decoder. The **Graph and Modbus apps now consume this stream** instead of decoding in TypeScript — which also fixed a Graph bug where Modbus registers were read little-endian (e.g. a temperature showing -2867.2 instead of 14.4), since both apps now go through the one decoder. (The Decoder app cutover and removal of the TS decode engine follow.) [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src-tauri/src/ws/protocol.rs](src-tauri/src/ws/protocol.rs), [src/stores/sessionStore.ts](src/stores/sessionStore.ts), [src/apps/graph/Graph.tsx](src/apps/graph/Graph.tsx), [src/apps/modbus/Modbus.tsx](src/apps/modbus/Modbus.tsx), [docs/session-flow.md](docs/session-flow.md).
