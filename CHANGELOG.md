...
- **Modbus catalogues now model slaves as nodes that own the device address**: a Modbus register (formerly mislabelled a "frame") is attributed to a slave **node** (`[node.<name>]` with `device_address`) via a new `node_address = N` key — the register names its slave's address directly and is matched to a node by that address, so renaming a node never breaks a register reference. The device address moved off `[meta.modbus]` (the old single catalog-wide default) and onto each node, so one catalogue can poll several slaves at their own addresses — the per-poll address now flows all the way through to the TCP/RTU drivers (TCP switches slave per poll over one socket via `set_slave`; RTU bakes it into each request) and the MCP/headless poll builder. The catalogue editor drops the per-register "Device Address / Use default" field for a **Slave** picker, the sidebar's Nodes view groups registers by slave, the node detail view is protocol-aware (a Modbus node shows its device address and "Registers on this Slave"), and the add/edit-node dialogs gained a device-address field. Register detail/badges now read **"Register"** (one signal) or **"Register Group"** (several). Legacy catalogues with a `[meta.modbus].device_address` and no nodes still load and poll correctly: a slave node is synthesised from the legacy address on parse and the orphaned registers are attached to it. [src-tauri/src/io/modbus_tcp/reader.rs](src-tauri/src/io/modbus_tcp/reader.rs), [src-tauri/src/io/modbus_rtu/reader.rs](src-tauri/src/io/modbus_rtu/reader.rs), [src-tauri/src/mcp/session.rs](src-tauri/src/mcp/session.rs), [src/utils/modbusPollBuilder.ts](src/utils/modbusPollBuilder.ts), [src/apps/catalog/views/NodeView.tsx](src/apps/catalog/views/NodeView.tsx), [src/apps/catalog/views/protocol-editors/ModbusConfigSection.tsx](src/apps/catalog/views/protocol-editors/ModbusConfigSection.tsx).
