...
- **The catalogue-sharing icons are one vocabulary rather than a dozen accumulated choices**: the same idea was drawn differently depending on which screen you were on. Pulling an upstream change showed you a download arrow for the *state*, a refresh arrow on the **Pull** menu item, a refresh arrow again on the toolbar's **Review upstream update**, and a download arrow on **Apply** in the review dialog — four steps of one flow, three glyphs, none agreeing with the status that started it. `GitBranch` meant "repository" in two places and "branch" in four. Four different glyphs meant "something is wrong". And the sync-status icon added earlier in this cycle collided outright with **Push**: lucide re-exports `UploadCloud` as an alias of `CloudUpload`, so "Local ahead" and "Push local changes…" were literally the same picture on the same Settings row — a collision invisible to a search because the two names differ. Everything now follows one rule: **a glyph says either where things stand or what a button does, never both.** State is a noun and takes arrows — `↑` local ahead, `↓` remote ahead, `↕` diverged — which is what those words already mean. Transfer is a verb and takes clouds, so pull is a cloud with a down arrow from the status badge through to the button that applies it, and the refresh arrow now means only "check for updates", which is a poll that writes nothing. Repository objects take git marks, with `GitBranch` reduced to branches alone. Severity is carried by colour, so there is **one alert glyph** and the shared alert component no longer lets each caller pick a picture — the exception being the secret scan, which keeps a shield because a security finding is not a warning box. Settings rows now show the status glyph *beside* its label, so the list where there is room to write it teaches the picker where there is not. Also fixed along the way: a Save button and a favourite toggle shared the star on one screen, and "Open on GitHub" showed a generic external-link arrow while the button beside it showed the GitHub mark. The vocabulary lives in one file with a test asserting no two names resolve to the same glyph — the only check that can see through a lucide alias, and the one that would have caught the original collision. Reference: [docs/catalog-sharing.md](docs/catalog-sharing.md) (§ The icon vocabulary). [src/components/catalogIcons.ts](src/components/catalogIcons.ts), [src/components/Alert.tsx](src/components/Alert.tsx), [src/components/catalogSyncPresentation.tsx](src/components/catalogSyncPresentation.tsx), [src/apps/settings/views/CatalogsView.tsx](src/apps/settings/views/CatalogsView.tsx), [src/dialogs/catalog-picker/CatalogPickerDialog.tsx](src/dialogs/catalog-picker/CatalogPickerDialog.tsx).

- **The Push catalogue dialog is now tabbed, and shows you the diff before you push**: publishing used to be a single stack of everything — repository, commit message, plan card, four kinds of alert — with the branch, path and pull-request controls folded into a collapsed "Branch, path and pull request" section at the bottom. That section could **silently disable the Push button**: ticking "Open a pull request" without naming a branch is refused, and the control that caused it was behind a fold you had no reason to open. The dialog is now **Push · Branch & pull request · Changes**, with the repository, commit message and the `repo @ branch → path` target line pinned above the tabs, and every reason Push is off listed in a strip directly above the button — each with a **Fix on …** link to the tab that resolves it, and a dot on that tab. Reasons that no tab can fix, like "nothing to push", say so and offer no link. **The Changes tab is the substantive addition**: it shows the real diff between your saved file and the copy upstream, computed from the clone that preflight already fetched, so it costs a blob read rather than a request. It reports which ref it compared against (falling back to the base branch when the branch you named does not exist yet), whether this push would *create* the file, when that path last changed upstream and by whom, and warns when upstream has moved since you imported it — the one case where pushing replaces someone else's work. The branch field gains suggestions from the clone's real branches while staying free text, so naming a *new* branch still works. Also fixed: **an unchanged file is now refused before it becomes an empty commit**, enforced in the git layer where the tree and its parent are both already in hand, because the dialog can only warn about a branch it has diffed against. The diff is computed in Rust and returned as rendered rows — returning the two file texts instead would have shipped both files to the frontend and straight back over the WebSocket to be diffed by the function that was one call away. Reference: [docs/catalog-sharing.md](docs/catalog-sharing.md). [src-tauri/src/catalog_share/publish.rs](src-tauri/src/catalog_share/publish.rs), [src-tauri/src/catalog_share/git.rs](src-tauri/src/catalog_share/git.rs), [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src/apps/catalog/dialogs/publish/](src/apps/catalog/dialogs/publish/), [src/components/TabStrip.tsx](src/components/TabStrip.tsx).

- **The catalogue picker shows each catalogue's git status at a glance**: Select Catalog listed a title and a filename and said nothing about where that catalogue stood against its repository — you could not tell a purely local decoder from one tracked upstream, and certainly not one about to be overwritten from one with unpushed work. Each row now carries an icon: **Local only, In sync, Local ahead, Remote ahead, Diverged, File missing** or **Not checked**. All seven are shown rather than folded into four, because Diverged is the only state that needs a decision and must not read as a safe push, and because "not checked" is honest — a tracked catalogue's remote side stays unknown until Check for updates runs. **The status is computed in Rust and rides on every catalogue**, which is what lets the picker draw it without pulling in the sharing store: that picker is mounted by five panels — Catalog editor, Decoder, Dashboard, Query and the Data Source dialog — that otherwise never touch it. It is also nearly free where it now happens, since the decoder-directory scan already reads every file to extract its `[meta].name`. Moving the collapse to Rust makes it the single source of truth for real rather than by convention: the TypeScript copy is gone, Settings → Catalogs reads the same field the picker does, and the two raw states that used to be shipped alongside it — an open invitation to re-derive rather than ask — are no longer sent at all. The status also stays live: a check for updates, a push, a pull or forgetting a catalogue each rebuild the cached list, none of which touch a single file on disk, so nothing else would have noticed. Reference: [docs/catalog-sharing.md](docs/catalog-sharing.md). [src-tauri/src/catalog_share/registry.rs](src-tauri/src/catalog_share/registry.rs), [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src-tauri/src/catalog_share/mod.rs](src-tauri/src/catalog_share/mod.rs), [src/components/catalogSyncPresentation.tsx](src/components/catalogSyncPresentation.tsx), [src/dialogs/catalog-picker/CatalogPickerDialog.tsx](src/dialogs/catalog-picker/CatalogPickerDialog.tsx).

- **The MCP server now speaks protocol revision `2026-07-28`**: the revision published on 28 July 2026 is the largest break MCP has had — it removes the `initialize` handshake and protocol-level sessions entirely, so every request carries its own protocol version, client identity and capabilities, and adds a mandatory `server/discover` for clients that want to check compatibility up front. WireTAP is now on `rmcp` 3.x and serves it, **while continuing to answer the older `2025-11-25` handshake** — so a client that has not moved yet keeps working unchanged, and the two can even be in use at the same time. Three things improve for any client on the new revision. **Tool results are now structured**: every tool returns machine-readable `structuredContent` alongside the JSON text block it always returned, so a client no longer has to re-parse a string to get at a frame inventory or a byte profile. **Tools are annotated** with `readOnlyHint`, `destructiveHint` and `idempotentHint`, derived from the permission gate each tool sits behind — the twenty-seven read tools declare themselves read-only, `transmit_frame` and `modbus_write` declare themselves destructive, and a client can use that to decide what to run without asking and what to hold for confirmation. **`tools/list` carries a cache hint** (five minutes, private), which stops a chatty client re-fetching forty tool schemas on every turn. Also tightened while here: the transport now validates the `Origin` header, which is the DNS-rebinding defence the specification requires and which was previously off, so a web page can no longer reach the local server; the bearer-token check is constant-time; and the tool router is built once at startup rather than per request, which the new stateless transport would otherwise have made per-call work. Because the new revision has no disconnect signal at all, the Session Manager's MCP connect/disconnect entries are now driven by an activity window for stateless clients, while legacy clients keep their exact handshake-based entries. Not adopted, deliberately: resources, prompts, sampling, roots, elicitation, the Tasks extension and OAuth. Reference: [docs/mcp-analysis-tools.md](docs/mcp-analysis-tools.md). [src-tauri/src/mcp/mod.rs](src-tauri/src/mcp/mod.rs), [src-tauri/src/mcp/tools.rs](src-tauri/src/mcp/tools.rs).

- **Catalogue sharing now uses real git clones, and branches and pull requests are optional**: pulling and pushing a decoder used to be an emulation of git over the GitHub contents API — around 600 lines listing trees, reading blobs, creating refs and PUTing files one at a time. It was GitHub-only, it could not answer "am I ahead or behind?", and it forced *every* push through a new branch and a pull request, because the contents API was the only transport available and a PR was the only way to get a change anywhere you could not already push. Each saved repository is now **cloned with libgit2** into the app data directory: pull is a fetch, push is a commit plus a push, and what is left on disk is an ordinary git repository you can `cd` into and inspect with the `git` binary. **Pushing now goes straight to the branch the catalogue was pulled from, with no branch and no pull request** — the common case is a decoder going back to a repository you own, where both were ceremony around a one-file change. Naming a branch creates one; ticking "Open a pull request" opens one; neither is forced, not even when a fork is involved, since pushing to your own fork and stopping is a legitimate way to park work. Settings → Decoder Catalogs is rebuilt around this: each row shows **one coloured status badge** — Local only, In sync, Local ahead, Remote ahead, Diverged, File missing, Not checked — in place of the old scatter of provenance, sync and update pills, and the six flat icon buttons collapse into a single **⋮ menu** carrying Pull, Push, Open on GitHub, View pull request, Reveal the local clone, Duplicate, Rename, Stop tracking and Delete. **Pull is one click**: when upstream has moved and your copy has no edits of its own, it applies immediately with no dialog, and the diff review opens only when both sides moved and there is genuinely a decision to make. The clone is deliberately a transport and a mirror rather than a workspace — WireTAP never leaves it divergent, and there is no merge or conflict UI, because the file of record is the one in your decoder directory. Enumerating and reading catalogues now costs **no GitHub API requests at all**, which retires the tree cache, the raw-CDN fallback and the bounded-concurrency blob fetching that existed only to fit inside 60 requests an hour; ref disambiguation reads the clone's own refs, so it no longer needs a GitHub endpoint either. Also fixes three real defects found while rewriting: a catalogue published to a *different* repository than it was tracked against inherited the wrong remote path; a second push while a pull request was open could open a rival PR on a different branch that the branch-keyed lookup would never find; and an open PR against a different base branch was reported as the current one, silently discarding the base you were shown. libgit2 is vendored so no system libgit2 is needed, and its TLS backend differs per platform — SecureTransport on macOS and iOS, WinHTTP on Windows, OpenSSL on Linux. Reference: [docs/catalog-sharing.md](docs/catalog-sharing.md). [src-tauri/src/catalog_share/git.rs](src-tauri/src/catalog_share/git.rs), [src-tauri/src/catalog_share/publish.rs](src-tauri/src/catalog_share/publish.rs), [src-tauri/src/catalog_share/github.rs](src-tauri/src/catalog_share/github.rs), [src/components/OverflowMenu.tsx](src/components/OverflowMenu.tsx), [src/utils/catalogSync.ts](src/utils/catalogSync.ts), [src/apps/settings/views/CatalogsView.tsx](src/apps/settings/views/CatalogsView.tsx).

- **Saved repositories, with one starred as the publish default**: "Import from Git" is now **Repository** — a dialog that keeps the repositories you share catalogues with, instead of making you find the URL again every time. Save a repository from a pasted URL and it stays; star one and it becomes the default target when you publish. A saved entry can carry a branch and a catalogue directory, both inferred from the URL you pasted — so `…/tree/main/catalogs` captures the repository, the ref `main` and the folder `catalogs` in one step. Clicking a saved row browses it straight away, at that ref and folder, and publishing a *new* catalogue to that repository defaults to that directory instead of the generic `catalogs/`. The saved directory never overrides where a catalogue came from: one imported from `decoders/x.toml` still publishes back to `decoders/x.toml`, so an update stays an update rather than adding a second copy elsewhere in the repository. **Publishing now picks the repository from a dropdown** rather than a free-text URL field, defaulting to the starred one, with an "Add a repository by URL…" option that saves and selects in one step (distinct from the existing "New repository…", which creates one on GitHub). Creating a repository in-app now saves it too. This replaces the old recent-repositories list, which only ever recorded repositories you had *browsed* — publishing to one, or creating one, never added it, so the publish field's suggestions were reliably the wrong ones. Saved repositories are stored in `catalog-sources.json` in their own list, deliberately apart from the internal repository records that are garbage-collected when their last catalogue is forgotten. Note the saved branch applies to browsing and importing only; a push targets the ref the catalogue itself was pulled from (see the git-clone entry above). Reference: [docs/catalog-sharing.md](docs/catalog-sharing.md). [src-tauri/src/catalog_share/registry.rs](src-tauri/src/catalog_share/registry.rs), [src-tauri/src/catalog_share/mod.rs](src-tauri/src/catalog_share/mod.rs), [src/dialogs/catalog-share/RepositoryDialog.tsx](src/dialogs/catalog-share/RepositoryDialog.tsx), [src/apps/catalog/dialogs/PublishCatalogDialog.tsx](src/apps/catalog/dialogs/PublishCatalogDialog.tsx), [src/stores/catalogShareStore.ts](src/stores/catalogShareStore.ts).

- **Check tracked catalogues for upstream updates, and review before applying**: closes the collaboration loop — you can now see that someone else has moved a catalogue you imported, read exactly what changed, and take it. Settings → Catalogs gains **Check for updates**, which batches per repository *and ref* and short-circuits on the head commit, so an unchanged repository is checked without downloading its tree; the tree is listed **at the head commit** rather than the ref, so a moved head can never be served a stale cached listing (which would otherwise record the new head and hide the update until the next upstream commit). Open pull requests refresh in the same pass, so a merge shows up without a separate button, and everything learned is applied in one registry write. **An update never destroys local work.** If the local file is unchanged it is backed up to a hidden `.wiretap-backups/` directory and replaced; if it has your edits the backend *refuses* — re-checking both the sync state and the file's hash as reviewed, so a file edited mid-review is not clobbered — and the update instead loads into the Catalog editor as a reviewable, saveable diff, using the same buffer-versus-baseline mechanism a schema upgrade already uses. Nothing reaches disk until you save, and there is deliberately no merge UI. The diff itself is the existing Rust `catalog.diff` rendered by the existing `DiffView`. A new `useCatalogSources` hook joins the catalogue list with its Git provenance and reconciles both off the *same* `CatalogListChanged` push, which also fixes a catalogue imported from Settings showing no provenance badge until the panel remounted. Reference: [docs/catalog-sharing.md](docs/catalog-sharing.md). [src-tauri/src/catalog_share/mod.rs](src-tauri/src/catalog_share/mod.rs), [src-tauri/src/catalog_share/registry.rs](src-tauri/src/catalog_share/registry.rs), [src/apps/catalog/dialogs/CatalogUpdateDialog.tsx](src/apps/catalog/dialogs/CatalogUpdateDialog.tsx), [src/hooks/useCatalogSources.ts](src/hooks/useCatalogSources.ts), [src/stores/catalogEditorStore.ts](src/stores/catalogEditorStore.ts).

- **Publish a catalogue as a branch plus pull request** *(superseded before release by the git-clone rewrite above: branches and pull requests are now optional, and the commit goes over git rather than the contents API — the fork flow, secret scan, idempotency and PR tracking described here all still apply)*: the write half of Git sharing. Publish from the Catalog editor toolbar or per-row in Settings → Catalogs, and the app resolves where the commit belongs — straight into the upstream when you have push access, otherwise into your fork, creating and waiting for that fork if it does not exist yet (reading the fork's real name from GitHub, which appends `-1` when you already own a repository of that name). It then creates or reuses the branch, commits the file, and opens or updates the pull request. **Every step is idempotent**, so a mid-flight network failure is recovered by pressing Publish again: an existing fork is reused, a 422 on branch creation is a reuse, and an open pull request on the same branch is reported rather than duplicated — publishing a second edit adds a commit to the PR under review instead of opening a rival one. There is deliberately no rollback, since deleting a branch or fork on failure risks destroying work. A preflight step shows what publishing *would* do before anything is written: the exact bytes, whether a fork is needed, whether the target is public, the validation result (invalid catalogues are refused), and a **secret scan** over the file — GitHub and Slack tokens, AWS keys, private keys, JWTs, credential assignments, URLs with embedded credentials and VIN-shaped strings each require explicit acknowledgement, with copy noting that anything published publicly must be rotated rather than force-pushed away. The commit's returned blob SHA becomes the stored sync hash, so a published catalogue immediately reads as *committed*. Also supports commit-only mode (a branch, no PR), committing straight to the base branch when you have push access (off by default so review stays in the loop), creating a repository from the app (private by default, `auto_init` so the first publish has a branch to target), and reporting whether a pull request is still open or has **merged** — pushed and merged are different things, and for collaboration the difference is what people want to see. The GitHub token is validated against `GET /user` before being stored in the system keychain, under its own service so an unrelated IO-profile deletion cannot wipe it, and never crosses back over the IPC boundary. The write half sits behind a `PublishBackend` trait so an SSH-based backend can be added later without touching the frontend. Reference: [docs/catalog-sharing.md](docs/catalog-sharing.md). [src-tauri/src/catalog_share/publish.rs](src-tauri/src/catalog_share/publish.rs), [src-tauri/src/catalog_share/secrets.rs](src-tauri/src/catalog_share/secrets.rs), [src-tauri/src/catalog_share/auth.rs](src-tauri/src/catalog_share/auth.rs), [src-tauri/src/credentials.rs](src-tauri/src/credentials.rs), [src/apps/catalog/dialogs/PublishCatalogDialog.tsx](src/apps/catalog/dialogs/PublishCatalogDialog.tsx), [src/apps/settings/views/CatalogsView.tsx](src/apps/settings/views/CatalogsView.tsx).

- **Import catalogue decoders from a shared Git URL** *(superseded before release by the git-clone rewrite above: the provenance model, validation and collision rules below are unchanged, but files now arrive over git rather than the GitHub tree and blob APIs)*: reverse-engineering a device's protocol is collaborative work, but a catalogue was a lone `.toml` in the decoder directory with no way to hand it to someone or take their improvements short of emailing files. The catalogue picker gains **Import from Git** — paste any GitHub repository, directory, file, `raw.githubusercontent.com` or `git@` URL and the app lists the catalogues it finds, showing each one's `[meta].name`, protocol, frame count and validity *before* anything is written. Works anonymously (no account needed); a stored token only lifts the rate limit and unlocks private repositories. Imported files are validated first, written byte-exactly via temp-plus-rename, and never auto-attached to a session — the import preview states how many frames declare transmit intervals, since a stranger's catalogue can define traffic that would reach a live bus. A filename clash defaults to keeping both (`name-2.toml`) rather than overwriting hand-written work. Provenance is recorded in `catalog-sources.json` beside `settings.json`, keyed by the **git blob SHA-1** of the bytes last exchanged — the same identifier GitHub returns — so "is my copy committed?" is answered by hashing the local file with no network call, and an out-of-app rename is followed by content match. Everything runs over the GitHub REST API in Rust (the webview CSP blocks `api.github.com`), adding no git dependency and keeping the feature working on iOS. Reference: [docs/catalog-sharing.md](docs/catalog-sharing.md). [src-tauri/src/catalog_share/](src-tauri/src/catalog_share/), [src/api/catalogShare.ts](src/api/catalogShare.ts), [src/stores/catalogShareStore.ts](src/stores/catalogShareStore.ts), [src/apps/catalog/dialogs/ImportFromGitDialog.tsx](src/apps/catalog/dialogs/ImportFromGitDialog.tsx).
...
- **An MCP-opened session never started, and arrived undecoded**: `open_session` says it opens *and starts* a session, but for a recorded source — a PostgreSQL or WireTAP long-term store — it only created one. The session sat stopped with no capture and no frames, which from the outside is indistinguishable from a broken tool. The skip is deliberate for the desktop app (frames must not start flowing until the frontend has registered its listener), but headless there is no listener to race, so the session is now started too, and torn down rather than left holding the profile open if the start fails. It also **binds the profile's preferred decoder**, which is what a session needs to arrive decoded when `attach_source` later surfaces it in the Decoder, Dashboard or Query — previously that showed a raw undecoded stream, since nothing on the headless path had ever attached a catalogue. And `open_session` now takes **`start_time` / `end_time` / `speed` / `limit`**: a recorded source otherwise replays its entire archive from the beginning, so an agent that had used the query tools to *find* an interesting window had no way to then open a session on it. Each overrides the profile's own value for that one call without modifying the saved profile. Reference: [docs/session-flow.md](docs/session-flow.md) (§ Headless open). [src-tauri/src/mcp/session.rs](src-tauri/src/mcp/session.rs), [src-tauri/src/mcp/types.rs](src-tauri/src/mcp/types.rs).

- **A mirror frame whose source is multiplexed always showed Match, whatever its payload**: mirror validation compares only the bytes a mirror *inherits* from its source, and an inherited multiplexer is copied across wholesale without marking its case signals as inherited. A mirror of a frame that carries **only** a mux therefore inherited no bytes at all — and since "no compared byte differs" was read as "the payloads agree", the frame showed a green **Match** on its first sample and kept it regardless of what either side actually contained. In the SBR catalogue that is `0x008` and `0x00a`, whose sources hold the battery and manufacturer serial numbers. Such a mirror is now not tracked at all, so the badge shows nothing rather than passing a check that never ran. Comparing the multiplexed bytes themselves needs per-case inheritance the parser does not yet record, and is not attempted here. [wiretap-catalog `src/mirror.rs`].

- **A mirror frame's Match/Mismatch badge could be stale, and the Query app disagreed with it**: the Decoder's mirror validation kept its state in a frontend map that was created once and **never cleared** — not on loading a different catalogue, not on stopping or switching sessions, not on a replay, not on Clear frames. A latched Mismatch therefore survived all of those and persisted until the window was reloaded, and worse, the set of bytes being compared was computed from whichever catalogue happened to be loaded first, so after rebinding a catalogue the badge could be judging the *wrong bytes* with no sign that anything was amiss. The comparison now runs in Rust with its state built at `catalog.attach` and dropped at `catalog.detach`, so a verdict cannot outlive the catalogue that produced it — the stale case is structurally impossible rather than patched at each reset point. Clearing a capture now clears the comparison state too, which it never did: the retained samples used to re-assert the verdict you had just cleared, and after a replay rewound the timestamps sat outside the comparison window so a latched Mismatch could never be compared away. Separately, the **Query app's Mirror Validation compared whole payloads** while the Decoder compared only the bytes a mirror *inherits*, so the two legitimately disagreed: a catalogue that deliberately overrides a byte on the mirror (declaring a signal at the same position as its source — how a catalogue records that a byte carries different data on that copy) read as Match in the Decoder and as a mismatch on every single row in Query. Query, capture queries and the remote-backend passthrough now take the same inherited byte set, so both views answer the same question. Note the remote path filters after the gateway applied its row limit, so a heavily-restricted comparison against a `wiretap` profile can return fewer rows than the same query run against a local database. [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src-tauri/src/dbquery.rs](src-tauri/src/dbquery.rs), [src-tauri/src/capturequery.rs](src-tauri/src/capturequery.rs), [src/stores/decoderStore.ts](src/stores/decoderStore.ts).

- **A DNS failure was reported as a connection timeout, and some real errors showed no dialog at all**: connecting a GVRET TCP source to a hostname while its DNS server was unreachable — a VPN dropping is the everyday case — reported `[gvret_tcp(host:23)] connect timed out`, which points at the device and the network path rather than at name resolution. The cause is that `getaddrinfo` does **not** fail fast when the resolver itself is unreachable; it blocks until the C library gives up. Because the host and port were handed straight to `TcpStream::connect`, that stall happened *inside* the connect timeout, so the two failures were indistinguishable. Hostname resolution is now bounded and classified in one place, and every TCP source resolves first and then connects to the resolved address, so "I can't look this name up" and "I looked it up but couldn't reach it" are different messages — the first naming the host and pointing at the network or VPN. This also fixes two quieter variants of the same defect: **FrameLink** resolved *outside* its own connect timeout, so a stalled lookup bypassed the timeout entirely and the session hung indefinitely with no message; and the **Modbus TCP** reader, scanner, multi-source spawner and MCP tool had no bound on resolution at all. Their previous wording blamed the address (`Invalid server address …`), which is misleading when the address is perfectly valid and it is the resolver that is missing. Separately, and the reason this class of failure could stay invisible: the central stream-error handler suppressed any message containing the substring `not found`, which was meant for benign teardown races (a session or capture that has already gone) but also swallowed genuine faults — SocketCAN reporting `pkexec not found` on a machine without polkit, gs_usb reporting `Device not found`, and FrameLink reporting `Device '…' not found via discovery` — each leaving the session in the error state with no dialog. **Those three now raise a dialog where previously they failed silently**, which is the intended fix but is a visible change. Not addressed here: MQTT, PostgreSQL and the WireTAP backend API resolve inside their client libraries, where WireTAP cannot wrap the lookup, and none of the three currently sets a connect timeout. One behaviour change worth knowing: resolution now returns the first address rather than letting `TcpStream::connect` try every address a name resolves to, so a dual-stack host whose IPv6 path is black-holed no longer falls through to its IPv4 address — and the DNS bound is additive to the profile's own connect timeout rather than carved out of it. [src-tauri/src/io/net.rs](src-tauri/src/io/net.rs), [src-tauri/src/io/error.rs](src-tauri/src/io/error.rs), [src-tauri/src/io/gvret/tcp.rs](src-tauri/src/io/gvret/tcp.rs), [src/stores/sessionStore.ts](src/stores/sessionStore.ts).

- **The session log collapsed two MCP clients into one entry**: connect/disconnect entries were deduplicated against the previous entry on session id, event type and app name within a 100 ms window — a guard against the same event being logged twice. MCP events have no session id (the client is identified in the detail text), so two *different* clients connecting or disconnecting within 100 ms of each other were silently merged into a single line, leaving a connect with no matching disconnect or the reverse. The deduplication now also compares the detail text, so genuine repeats are still collapsed while distinct clients are not. Previously almost unreachable — a client could only announce itself one at a time — but routine now that idle clients are all expired together on one timer tick. [src/apps/session-manager/stores/sessionLogStore.ts](src/apps/session-manager/stores/sessionLogStore.ts).

- **The catalogue line diff could exhaust memory on a pathological file**: `lcs_diff` allocated an unbounded `n × m` table of `u32`, so a very long catalogue — pasted into text mode, written by MCP, or imported from a repository — could ask for gigabytes. It now degrades to a remove-all/add-all diff above 25M cells, which is coarse but truthful. [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs).
- **Catalogue saves could be observed half-written**: `save_catalog` wrote in place inside the directory the `notify` watcher watches, so a watcher-triggered rescan could read a truncated TOML and fall back to naming the catalogue after its filename. Saves now write to a temp file and rename, via a shared `write_file_atomically` helper. [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs).
- **Duplicate and rename accepted unsanitised catalogue filenames**: both commands joined a frontend-supplied name straight onto the decoder directory with no traversal guard and no `.toml` enforcement, while the MCP write tools were guarded. All four paths now share `sanitise_catalog_filename`, and its traversal check is shared with dashboard filenames as `reject_unsafe_filename`. [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src-tauri/src/dashboard.rs](src-tauri/src/dashboard.rs), [src-tauri/src/mcp/tools.rs](src-tauri/src/mcp/tools.rs).
...
- **Mirror validation moved into the catalogue crate, and the last TypeScript decode duplicates are gone**: comparing a `mirror_of` frame against its source was the only thing left in the Decoder store still computing anything from raw payload bytes — everything else had already become passthrough of the Rust decode stream. It is now `wiretap_catalog::mirror`, with per-session state in the WebSocket dispatcher and the verdict riding the existing `DecodedSignals` message, which removed about 130 lines from the store along with the plumbing that carried it through two views. One consequence is visible in behaviour: the Decoder used to push *unselected* mirror source frames through its decode path purely to keep the comparison fed, and no longer has to, because Rust observes every frame the session delivers rather than only the ones a view has asked for. Deleted while here: a second, complete mirror-inheritance resolver in `apps/catalog/protocols/can.ts` — a line-for-line duplicate of the Rust one that nothing had called since decoding moved across, but which still compiled and still read as live code — along with its serial and Modbus siblings and three unused mux-matching functions. Reference: [docs/session-flow.md](docs/session-flow.md). [wiretap-catalog `src/mirror.rs`], [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src/utils/mirrorBytes.ts](src/utils/mirrorBytes.ts), [src/apps/catalog/protocols/can.ts](src/apps/catalog/protocols/can.ts).

- **Saved repositories get a ⋮ menu, and the local clone is reachable**: the saved-repository rows in the Repositories dialog carried two bare icon buttons (pencil and trash), while Settings → Catalogs already put its per-row actions behind a `⋮` menu. The rows now match: the star stays as the "default for publishing" indicator and one-click toggle, and the menu carries **Favourite**, **Reveal the local clone**, **Properties** and **Remove**. Reveal opens the clone in the OS file manager, and is disabled — with a tooltip saying why — for a repository saved but never browsed, which has no clone yet. **Properties** absorbs the old edit dialog: read-only facts (owner/repo, URL, when it was saved, and the absolute clone path, each copyable) over the existing name / branch / directory form, so "where is this thing on disk?" is finally answerable from the UI. No new command was needed: the saved-repository listing now carries each entry's clone path and whether it exists (`SavedRepoView`), derived per call and still never persisted, so the fact arrives with the list it decorates instead of costing a round trip per row. Settings → Catalogs' own "Reveal the local clone" now shares one `revealRepoClone` helper, and says so when there is nothing cloned rather than appearing to do nothing. [src/dialogs/catalog-share/RepositoryDialog.tsx](src/dialogs/catalog-share/RepositoryDialog.tsx), [src-tauri/src/catalog_share/mod.rs](src-tauri/src/catalog_share/mod.rs), [src/api/catalogShare.ts](src/api/catalogShare.ts).

- **One catalogue picker everywhere, and every app can now get a catalogue it does not have**: the picker existed twice — the Catalog editor's copy carried New / Import / Repository, while Decoder, Dashboard, Query and the Data Source dialog used a footer-less fork of the same list, so from those four the only catalogues reachable were the ones already in the decoder directory. There is now a single `CatalogPickerDialog` offering **Import** and **Repository** in every app (**New** only where an editor can host the create flow), with the footer restyled as proper buttons on the Data Source dialog's footer surface. It sources its own catalogue list, decoder directory and repositories dialog, so a host only says what is selected and what to do when one is picked — five call sites lost their data wiring. A search field appears once there are more than eight catalogues. **Importing a file now copies it into the decoder directory** instead of opening it in place: a `.toml` picked from elsewhere used to be selected while remaining invisible to every catalogue list. The naming is done by a new `import_catalog` command rather than in the frontend, so a file import goes through the same sanitiser and the same non-destructive `-2` collision rule as the git import and the MCP write tools, resolved against the directory itself and written atomically. The Data Source dialog's decoder list also reconciles on `CatalogListChanged` now, where it previously fetched once per open and could not see a catalogue imported from within itself. [src/dialogs/catalog-picker/](src/dialogs/catalog-picker/), [src/dialogs/io-source-picker/DecoderPicker.tsx](src/dialogs/io-source-picker/DecoderPicker.tsx), [src/apps/catalog/CatalogEditor.tsx](src/apps/catalog/CatalogEditor.tsx).
...
- **Bundled example decoders are no longer installed**: WireTAP shipped two sample catalogues (`sbrxxx.toml` and `sungrow_shx.toml`) as bundle resources and copied them into your decoder directory at startup. Despite reading as a first-run convenience it ran on **every** launch, guarded only by whether a file of the same name already existed — so deleting or renaming a sample brought it back on the next start, with no way to decline it. The copies also drifted from the catalogues they were cut from — the iOS copy of `sungrow_shx.toml` had been stale since March, still on the pre-`[node]` schema — which made the samples a poor first impression of the format. Nothing is copied now: a new install starts with an empty decoder directory, and decoders arrive either from the Catalog editor or over git catalogue sharing, which is a far better answer to "where do I get one" than two frozen files (see the entries above). **Catalogues already in your decoder directory are untouched** — this only stops new files being written, and the samples that were previously installed remain exactly as they are, yours to keep, edit or delete. `sbrxxx.toml` is retained in the repository as a parser test fixture only; it is not packaged into the application. [src-tauri/src/settings.rs](src-tauri/src/settings.rs), [src-tauri/src/lib.rs](src-tauri/src/lib.rs), [src-tauri/tauri.conf.json](src-tauri/tauri.conf.json), [src-tauri/gen/apple/project.yml](src-tauri/gen/apple/project.yml).
...
- **Catalog Editor Diff view was hard to read**: the old/new line-number gutters used `minWidth` without `tabular-nums`, so the two number columns drifted out of vertical alignment, and the diff body used `whitespace-pre-wrap` + `break-all`, which wrapped long lines and broke them mid-token — destroying the visual indentation of nested TOML. The rows were rebuilt with fixed-width, right-aligned, `tabular-nums` number columns; the `+`/`-` marker is folded into a sticky (pinned) gutter so every row's content starts at the same column and the gutter stays put while the body scrolls horizontally (`whitespace-pre`, `tab-size: 4`) instead of wrapping — indentation is now preserved exactly. The gutter is painted opaque (a sticky element needs an opaque background, and the dark-theme status tints are translucent) with a coloured marker and left accent to signal the change side. [src/apps/catalog/views/DiffView.tsx](src/apps/catalog/views/DiffView.tsx).

- **Session roster entries were rebuilt on every reconcile when a session had no attached catalogue**: `reconcileKnownSessions` normalises an omitted `catalogPath` to `null` but compared it against a possibly-`undefined` existing value, so `undefined !== null` always read as a change and replaced the entry object each pass (needless churn and re-renders). The existing side is now normalised with `?? null` before the comparison. [src/stores/sessionRoster.ts](src/stores/sessionRoster.ts).

- **Decoder showed one shared byte row for multiplexed frames**: the hex/ASCII byte row rendered once per frame from a single `rawBytes` array, which was last-writer-wins across mux occurrences — so a multiplexed frame (e.g. `0x71F` SBR module serials) showed only the most recently received payload for *every* mux group, not the payload that produced each group. The decoded WebSocket stream (`DecodedSignals` 0x14) now carries the raw payload each decode came from, and the frontend stores it per mux value (`rawBytesByMux`), so each `Mux N` group renders its own hex/ASCII row. Non-mux frames are unchanged; a group falls back to the frame-level bytes until its per-mux payload arrives. [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src/stores/decoderStore.ts](src/stores/decoderStore.ts), [src/apps/decoder/views/DecoderFramesView.tsx](src/apps/decoder/views/DecoderFramesView.tsx), [src/services/wsProtocol.ts](src/services/wsProtocol.ts).
...
- **Session-subscriber tracking unified into a global open-app registry**: `IOSession` no longer holds a `subscribers` map or `joiner_count` — the per-session subscriber list and count are now *derived* from a global `APP_REGISTRY`, where each open session-aware app instance records the single session it's attached to (`session_id: Option<String>`). That one field makes the one-subscriber-one-session invariant automatic (attaching overwrites it), and subscriber ids are now deterministic (`${windowLabel}_${appName}`, one per window+app). Last-subscriber-leaves teardown, the suspension/grace heartbeat sweep, and window-close pruning all run off the derived count through a shared `teardown_session_if_empty`. The unused legacy `join_reader_session`/`leave_reader_session` joiner path was removed, and the per-window `focusStore` was slimmed to just the focused panel. No user-visible behaviour change. [src-tauri/src/io/mod.rs](src-tauri/src/io/mod.rs), [src-tauri/src/sessions.rs](src-tauri/src/sessions.rs), [src/hooks/useIOSession.ts](src/hooks/useIOSession.ts), [src/stores/focusStore.ts](src/stores/focusStore.ts).

- **Stop-to-capture decision moved into Rust**: the frontend `stopWatch` used to branch on the source's temporal mode (realtime → stop-and-switch-all-listeners with a suspend fallback; recorded → suspend + switch-to-capture-replay). A new `session_stop_to_capture` command owns that decision (and the fallbacks), so the frontend just calls it. [src-tauri/src/sessions.rs](src-tauri/src/sessions.rs), [src/hooks/useIOSessionManager.ts](src/hooks/useIOSessionManager.ts).
...
- **Anonymous feature-usage telemetry (separate opt-in) to see which features are actually used**: WireTAP already sent anonymous crash reports via Sentry; a second, independent opt-in now records anonymous *usage* events so we can make evidence-based calls like whether to retire a rarely-used IO source (e.g. Postgres). It's genuinely separate from crash reporting — its own one-time consent prompt (shown after the crash prompt; existing crash-consenters see only this one) and its own Settings → Privacy toggle, both defaulting off. Usage events are emitted as Sentry **structured logs from the Rust backend** ([src-tauri/src/telemetry.rs](src-tauri/src/telemetry.rs)), which sends over native HTTPS — bypassing the webview CSP and flush/lifecycle quirks that made frontend emission unreliable — using a logs-only client (backend panics are dropped in `before_send`; crash reporting stays in the frontend `@sentry/react` SDK). Every emit is gated in Rust on the usage opt-in via `load_settings_sync`. Each log carries a random anonymous per-install id (`install_id`) for count-unique of distinct installs, plus the OS (`std::env::consts::OS`) for a platform breakdown. Two signals are instrumented: **IO source starts** — emitted natively where the session is created and the source `kind` is known (`create_reader_session` covers Postgres/wiretap/MCP, `create_multi_source_session` covers realtime kinds deduped across buses, plus capture replay) — and **app-panel opens**, bridged from the main layout through a `track_feature_usage` command (excluding boot-time layout restores). The DSN is handed to Rust from the Vite `.env` at startup (`telemetry_init`), never hardcoded. Logs carry only low-cardinality feature names, the OS, and the install id — never CAN data, connection details, or personal data. A single shared `ConsentDialog` renders both the crash and usage prompts. [src-tauri/src/telemetry.rs](src-tauri/src/telemetry.rs), [src-tauri/src/sessions.rs](src-tauri/src/sessions.rs), [src-tauri/src/lib.rs](src-tauri/src/lib.rs), [src/api/telemetry.ts](src/api/telemetry.ts), [src/WireTAP.tsx](src/WireTAP.tsx), [src/main.tsx](src/main.tsx), [src/dialogs/ConsentDialog.tsx](src/dialogs/ConsentDialog.tsx), [src/apps/settings/views/PrivacyView.tsx](src/apps/settings/views/PrivacyView.tsx), [src/components/MainLayout.tsx](src/components/MainLayout.tsx), [src-tauri/src/settings.rs](src-tauri/src/settings.rs).

- **Session Manager graph shows open apps across every window**: the "Visual" tab's app nodes previously came from a per-window store, so each window only listed the apps open in *that* window. A Rust-owned open-app registry now tracks every open session-aware app instance across all windows and broadcasts roster changes the same way sessions are broadcast — a Tauri emit plus a new WS channel-0 `OpenAppsChanged` (0x17) message — which each window reconciles via a `list_open_apps()` query (mirroring the session-roster sync). Open panels register from Dockview's panel lifecycle, so even tabs that have never been activated appear; attaching to a source flips a node from unconnected (dashed) to connected. App nodes show a cosmetic per-instance id (e.g. `decoder_a3f9`), App Details gained a **Window** field, and the OS title bar shows which window you're in (`WireTAP — main`, `WireTAP — 1`; the primary window — internally `dashboard` — is shown as `main`). [src-tauri/src/io/mod.rs](src-tauri/src/io/mod.rs), [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src/stores/openAppsStore.ts](src/stores/openAppsStore.ts), [src/hooks/useOpenAppsSync.ts](src/hooks/useOpenAppsSync.ts), [src/components/MainLayout.tsx](src/components/MainLayout.tsx), [src/apps/session-manager/utils/layoutUtils.ts](src/apps/session-manager/utils/layoutUtils.ts), [src/utils/windowName.ts](src/utils/windowName.ts).
...
- **"Leave session" and "Stop session" controls (per-app vs. all-apps capture review)**: the session menu's single "Stop & review capture" item is split into three explicit exit controls (alongside the existing Destroy). **Leave session** detaches only the calling app and hands it a frozen snapshot of the capture (named `{capture}_{n}`, so repeated leaves stay unique) to review, while any other apps on the session keep streaming live — a new Rust `session_leave_to_capture` command copies the capture, unregisters just that subscriber and broadcasts `subscriber-evicted`. **Stop session** stops the shared source so every connected app switches to reviewing the same capture. The heavy lifting is in Rust; the frontend reacts to the existing broadcasts, and the redundant stop-alias "Pause" on realtime apps was retired in favour of Stop. [src-tauri/src/io/mod.rs](src-tauri/src/io/mod.rs), [src-tauri/src/sessions.rs](src-tauri/src/sessions.rs), [src/components/SessionControls.tsx](src/components/SessionControls.tsx), [src/hooks/useIOSessionManager.ts](src/hooks/useIOSessionManager.ts).

- **Session-picker activity ripple (replaces the "N frames" readout)**: the static frame-count text in the top bar is gone; the session status dot now emits a sonar-style ripple whose cadence scales with the live frame-arrival rate (log-mapped, since CAN rates span ~10–10000 fps; static when paused/stopped or the bus is quiet). It lives in the shared session picker, so every app's top bar shows it. [src/components/SessionControls.tsx](src/components/SessionControls.tsx), [src/WireTAP.css](src/WireTAP.css).

### Fixed

- **Signal notes disappeared after editing a signal via the tree/detail view**: the Catalog Editor's signal form is populated from the resolved model, whose `Signal` had no `notes` field, so editing a signal (the path mux-nested signals go through) loaded an empty Notes box and saving rewrote the signal without its note — silently deleting it from the TOML. Signal-level `notes` now flow through parser and model (the `wiretap-catalog` `Signal` gains a `notes` field populated via `parse_notes`; `signalProps` emits it), so notes survive the edit/save round-trip. [src/types/catalogModel.ts](src/types/catalogModel.ts), [src/apps/catalog/tree/catalogToTree.ts](src/apps/catalog/tree/catalogToTree.ts).

- **Decoder catalogue list could show empty at startup before populating a moment later**: `list_catalogs` re-walked the decoder directory on every call, and the decode-aware apps each read it once on their own mount (across windows), so the picker showed empty until each async read resolved — and the directory was re-scanned (and the duplicate-display-name warning re-logged) once per consumer. The list is now owned by a backend `CatalogCache` warmed once during `setup` (after the example-decoder seed) and served from memory, so the first read is always populated. It rebuilds on catalogue mutations (`save`/`duplicate`/`rename`/`delete_catalog`), on a `decoder_dir` change in `save_settings`, and — on desktop — via a `notify` filesystem watcher (debounced) that catches `.toml` files added or edited outside the app. Each rebuild broadcasts a global `CatalogListChanged` (0x18) over the WebSocket (not a Tauri event); the shared `useCatalogList` hook (Decoder, Dashboard, Query, Catalog Editor) reconciles via `list_catalogs` on push + mount + reconnect, mirroring `useOpenAppsSync`. [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src-tauri/src/ws/protocol.rs](src-tauri/src/ws/protocol.rs), [src-tauri/src/settings.rs](src-tauri/src/settings.rs), [src/hooks/useCatalogList.ts](src/hooks/useCatalogList.ts).

- **Dashboard re-attached the decoder ~20×/s (a "session decoder changed externally" log flood)**: the dashboard kept a writable local catalog path that its catalogue loader wrote back, racing the `useSessionCatalog` mirror that copies the session's path in — the two never converged, so attach+parse re-ran every cycle at the attach round-trip cadence (also driving heavy re-renders). The session's attached decoder is now Rust-authoritative: recorded on `catalog.attach`, reported as `ActiveSessionInfo.catalog_path`, and reconciled one-way; the dashboard loader no longer writes it back. [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src-tauri/src/io/mod.rs](src-tauri/src/io/mod.rs), [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src/stores/dashboardStore.ts](src/stores/dashboardStore.ts), [src/stores/sessionRoster.ts](src/stores/sessionRoster.ts), [src/hooks/useSessionCatalog.ts](src/hooks/useSessionCatalog.ts).

- **Dashboard line-chart hover tooltip flickered between a styled and an unstyled rendering**: an orphaned uPlot instance could leave a second, stale tooltip node in the DOM. The chart now has a single self-cleaning create/destroy lifecycle (the resize observer recreates through it instead of newing up its own chart), and the body-appended tooltip/measurement overlays are deduped on init. [src/apps/dashboard/views/panels/line-chart/LineChartPanel.tsx](src/apps/dashboard/views/panels/line-chart/LineChartPanel.tsx), [src/apps/dashboard/views/panels/line-chart/chartPlugins.ts](src/apps/dashboard/views/panels/line-chart/chartPlugins.ts).

- **A Modbus device stayed busy after its session was torn down, so an immediate restart failed**: interval poll tasks only checked their cancel flag after the next tick, so a long-interval Modbus poll (e.g. every 30s) kept its connection open for up to a full interval after `stop()` — and because the broker's `stop()` awaits those tasks, teardown stalled and left the device locked, colliding with the restart (File exists / Broken pipe / Operation now in progress). The shared `Cadence` primitive now notices a set cancel flag within 100ms, without changing tick timing (the timer still wins the select when its deadline is sooner), so it also helps transmit-repeat tasks stop snappily. [src-tauri/src/io/periodic.rs](src-tauri/src/io/periodic.rs).

- **No decoded signals after leaving a session and replaying its capture**: decode runs only on frames past a forward-only per-session send offset, so a catalogue bound *after* a capture replay had already streamed its frames left them showing "No signals decoded". `catalog.attach` now re-decodes the frames already delivered to the client and pushes their signals (without re-sending the frames). [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs).

- **A session could be left running forever after an app switched sources**: a session is destroyed only when its *own* subscriber count reaches zero, but the backend let the same subscriber id register on more than one session at once. When an app moved between sessions (e.g. switching the Decoder from a live Modbus source to a capture replay), the frontend's best-effort "leave" could lose the race — the old session kept its subscriber and never tore down, so it stayed running and polling the device indefinitely while showing as a second session bound to the one app. `register_subscriber` now enforces one-subscriber-one-session: registering on a session removes that subscriber from every other session, and a session emptied by the move is destroyed (stop source, orphan capture, lifecycle event) exactly like a normal last-subscriber-leaves teardown — authoritative in Rust, so no frontend timing can orphan a session. [src-tauri/src/io/mod.rs](src-tauri/src/io/mod.rs).

- **Decoder showed no signals on a freshly-created session until a catalog was re-picked**: distinct from the 0.8.1 catalog-*list* fix, the per-session auto-select (which builds the catalog path from a source profile's `preferred_catalog`) ran before the decoder directory had resolved from settings. With an empty directory `buildCatalogPath` produced a bare filename, the file read failed, and the effect's own "already set" guard then stopped it ever re-running — so decoding stayed empty until the user picked a catalog by hand. The auto-select now waits for the directory to resolve and re-runs once it does, building an absolute path the first time; the same fix was applied to the Dashboard and Query auto-select paths. `list_catalogs` also logs a non-fatal warning when two catalogs share a display name (selection is keyed by filename, so it's harmless, but it explains an otherwise confusing picker). [src/apps/decoder/Decoder.tsx](src/apps/decoder/Decoder.tsx), [src/apps/dashboard/Dashboard.tsx](src/apps/dashboard/Dashboard.tsx), [src/apps/query/Query.tsx](src/apps/query/Query.tsx), [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs).
...
- **WireTAP Backend: a Dockerised TimescaleDB + API gateway so nothing connects to the database directly**: a new self-contained stack in [tools/wiretap-backend/](tools/wiretap-backend/) (TimescaleDB + a Rust/axum `wiretap-backend` service + an admin UI, optional pgBackRest) becomes the single owner of the long-term capture database. The backend exposes the binary ingest protocol (TCP 9323) for devices and an HTTP API (8423) for everything else, so microcontrollers, the Pi `wiretap-server`, and the desktop app all authenticate with API keys instead of holding PostgreSQL credentials. Ingest is **ACK-after-write**: each batch is written to PostgreSQL before the device is acknowledged, so a database outage back-pressures the device into its own disk cache and nothing is buffered in gateway RAM (verified zero-loss across a live outage/restore drill). It manages **one database per capture** and auto-creates a database when a new ingestor announces an unknown one in its HELLO (the protocol gained an optional database field); an admin can delete a capture database (refused while it's actively ingested or for the default/meta database). API keys (roles read/ingest/admin, optionally database-pinned) are stored hashed in the database and can be revoked, restored or permanently deleted from a thin built-in admin UI at `/admin` (keys, databases, live ingest sessions, query activity, health), with a break-glass `WIRETAP_ADMIN_KEY`. The HTTP API ports every Query-app analytical query plus a keyset-cursor frame-replay endpoint, capture import, and pg_stat_activity control; its SQL is verified against direct-PostgreSQL ground truth. See [tools/wiretap-backend/README.md](tools/wiretap-backend/README.md) and [docs/ingest-protocol.md](docs/ingest-protocol.md).

- **Desktop: `wiretap` IO profile kind talks to the backend over HTTP**: a new profile kind (URL + API key + database) routes all PostgreSQL-backed features — the Query app's analytical queries, MCP analysis tools, and recorded-source replay (the new `BackendApiSource` streams frames via the HTTP cursor with the same pacing as the direct PostgreSQL source) — through the gateway instead of a direct database connection. The existing `postgres` kind and its direct-SQL path are unchanged, so this is additive. Profile editor, Query app, time-range/bookmark filters and default-speed handling all recognise the new kind. [src-tauri/src/apiclient.rs](src-tauri/src/apiclient.rs), [src-tauri/src/io/recorded/backend_api.rs](src-tauri/src/io/recorded/backend_api.rs), [src-tauri/src/dbquery.rs](src-tauri/src/dbquery.rs), [src/apps/settings/dialogs/IOProfileDialog.tsx](src/apps/settings/dialogs/IOProfileDialog.tsx).

- **Desktop: send a local capture to the backend for permanent storage**: a "Send to backend" action on each capture in the IO source picker uploads its frames to a chosen (or new) backend database over the import API, streamed in chunks with a progress bar. [src/dialogs/SendCaptureToBackendDialog.tsx](src/dialogs/SendCaptureToBackendDialog.tsx), [src-tauri/src/apiclient.rs](src-tauri/src/apiclient.rs).

- **wiretap-server forward mode**: the Pi server can push frames to a backend gateway over the binary ingest protocol (`[forward]` config) instead of writing to PostgreSQL directly, so it no longer needs database credentials. Implemented as a `ForwardSink` subclass that reuses the existing queue, batching and SQLite disk-cache machinery — a gateway outage caches frames locally and drains them in order on reconnect (verified zero-loss, zero-duplicate across a mid-stream outage). Mutually exclusive with `[postgres]`; GVRET live bridging is unaffected. [tools/wiretap-server/wiretap-server.py](tools/wiretap-server/wiretap-server.py).

- **Binary TCP ingest API for microcontroller capture devices**: wiretap-server gained a second listener speaking a compact binary protocol (fixed little-endian layouts, CRC-32 framing, batch ACKs with backpressure, optional shared-token auth) so ESP32-class devices can push CAN frame batches over the network without speaking libpq. A classic 8-byte frame costs 18 bytes on the wire; clockless devices can send relative timestamps and the server back-dates from arrival time. Ingested frames flow through the existing batcher, COPY writer and SQLite outage cache; setting `[server].iface = ""` runs an ingest-only deployment with no local CAN hardware. Protocol spec in [docs/ingest-protocol.md](docs/ingest-protocol.md); reference client + 22-case loopback suite in [tools/wiretap-server/test_ingest_client.py](tools/wiretap-server/test_ingest_client.py); server in [tools/wiretap-server/wiretap-server.py](tools/wiretap-server/wiretap-server.py).

- **TimescaleDB schema + online migration for long-term capture archives**: `init_schema.sql` now creates `can_frame` as a hypertable (1-day chunks) with columnar compression segmented by arbitration id (compress after 7 days), drops the stored `id_hex`/`data_hex` generated columns (verified unread — the convenience views compute them on the fly) and the `row_id` serial PK nothing reads, and prunes six indexes down to the two that actual query predicates use. An hourly continuous aggregate (`can_frame_hourly`) makes frame-inventory rollups near-instant. [tools/wiretap-server/migrate_to_timescale.py](tools/wiretap-server/migrate_to_timescale.py) moves an existing archive into a TimescaleDB hypertable on another server (e.g. the backend container) — day-by-day binary COPY with count+checksum validation on both ends, compress-as-you-go, and resumable (completed days are skipped); the hourly rollup is refreshed over the imported range at the end. [tools/wiretap-server/init_schema.sql](tools/wiretap-server/init_schema.sql).

- **Catalogue format migrations surface as a saveable diff on load**: when the editor opens a catalogue whose schema is upgraded on parse, the upgrade is now applied to the *text* in Rust (comment-preserving, via a new `catalog.migrate` command in the `wiretap-catalog` crate) and loaded as the working buffer, while the on-disk original stays the diff baseline. The migration therefore shows up in the Text/Diff view, lights up the unsaved-changes indicator, and a dismissible banner explains what changed and prompts a save — instead of the upgrade being an invisible in-memory transform lost on close. Two migrations run today: (1) a legacy `[meta.modbus].device_address` with no nodes becomes a synthesised `[node."Slave N"]` with the registers referencing it by `node_address`; (2) a frame's poll/tx interval is flattened from the legacy `[tx]` sub-table (`tx.interval_ms`/`tx.interval`) to the canonical top-level `interval_ms`. The parser accepts both spellings, but the editor now writes (and migrates to) the flat `interval_ms`. Migrations are idempotent and share their logic with the parser's in-memory migration so the two cannot drift. [src/apps/catalog/CatalogEditor.tsx](src/apps/catalog/CatalogEditor.tsx), [src/stores/catalogEditorStore.ts](src/stores/catalogEditorStore.ts), [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs).

- **Catalog Text mode has a diff view, and the unsaved-changes indicator is Rust-computed**: a new Edit/Diff toggle in the catalogue editor's Text mode shows an inline unified diff (`+`/`-` with line-number gutters) of the working buffer against the last save. The diff and the dirty flag are both computed by a Rust `catalog.diff` command, so the save-icon indicator and the diff view are driven from one source. [src/apps/catalog/views/DiffView.tsx](src/apps/catalog/views/DiffView.tsx), [src/apps/catalog/CatalogEditor.tsx](src/apps/catalog/CatalogEditor.tsx), [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs).

- **Catalog `display` hints + "Add as Instruments"**: a signal can carry a `display` hint in its catalog (shorthand `display = "rotary"`, or a table `display = { widget = "level-bar", orientation = "vertical" }`); the Rust crate ignores the unknown key, so the frontend recovers it by re-parsing the catalog's raw TOML. A new wand-icon top-bar action opens the signal picker in a standalone "Add as Instruments" mode that creates one pre-configured widget panel per selected signal — using the explicit hint, else inferring from the signal's metadata (enum → indicator, bounded + angular unit → rotary, bounded → level bar, otherwise line chart). `min`/`max` are now surfaced from the catalog crate model. [src/apps/dashboard/widgets/displayHints.ts](src/apps/dashboard/widgets/displayHints.ts), [src/apps/dashboard/widgets/autoWidget.ts](src/apps/dashboard/widgets/autoWidget.ts), [src/apps/dashboard/dialogs/SignalPickerDialog.tsx](src/apps/dashboard/dialogs/SignalPickerDialog.tsx).

- **Visualisation widget registry + six new instrument primitives in the Dashboard app**: the Dashboard app's panel types are now a registry (`widgetMeta.ts` + `registry.tsx`) rather than a hardcoded conditional, so the add-panel menu, rendering, image-export surface routing and per-type defaults all derive from one table. Six widgets join the existing line-chart/gauge/list/flow/heatmap/histogram: **indicator** (icon-state — maps an enum value to a lucide glyph + colour + brightness), **rotary / steering-wheel** (a bounded scalar → rotation), **level bar + readout**, **bitfield** (per-bit on/off of a frame's latest bytes), plus two scriptable surfaces (below). The widget config dialog gained range fields for rotary/level-bar, a frame-id picker for bitfield, and a code editor for the custom widgets. [src/apps/dashboard/widgets/](src/apps/dashboard/widgets/), [src/apps/dashboard/views/DashboardGrid.tsx](src/apps/dashboard/views/DashboardGrid.tsx), [src/stores/dashboardStore.ts](src/stores/dashboardStore.ts).

- **Animated, scriptable custom-canvas and custom-SVG widgets, sandboxed in a Web Worker**: a custom widget runs author-supplied draw code — `(ctx, { signals, width, height, time, dt })` for canvas, or `(signals, { … }) → svg markup` for SVG — driven by a main-thread requestAnimationFrame loop so it animates independent of data rate. The code runs in a dedicated Worker with no DOM, no Tauri IPC and network/storage globals scrubbed; canvas frames return an `ImageBitmap` (so PNG export still works), returned SVG markup is DOMPurify-sanitised before injection, and a host watchdog terminates code that misses its per-frame deadline. Custom-SVG also has a declarative scene-graph mode (composed gauges/needles/lamps/bars/text) that executes no code at all. The author-code path requires `'unsafe-eval'` in the app CSP — the Worker sandbox remains the real boundary. [src/apps/dashboard/widgets/customWorker.ts](src/apps/dashboard/widgets/customWorker.ts), [src/apps/dashboard/widgets/customWorkerHost.ts](src/apps/dashboard/widgets/customWorkerHost.ts), [src-tauri/tauri.conf.json](src-tauri/tauri.conf.json).

- **Standalone dashboard artifacts (`*.dashboard.json`)**: panels + layout can be saved as a shareable file in a `dashboards/` subdirectory of the decoder directory and reopened from the Dashboard → Layouts menu, alongside the existing in-store named layouts. New Rust list/open/save commands and a frontend schema (`wiretap.dashboard/1`). [src-tauri/src/dashboard.rs](src-tauri/src/dashboard.rs), [src/utils/dashboards.ts](src/utils/dashboards.ts), [src/api/dashboards.ts](src/api/dashboards.ts).

- **MCP dashboard authoring + UI control, behind two new permissions**: `create_dashboard` validates and writes a dashboard artifact (embedded custom-widget code is stored opaque and never executed server-side), and `open_app` opens or focuses any app/panel in the running window — optionally loading a dashboard first via a new `ui.openPanel` reverse-RPC bridge method (reusing the existing `windowCommunication.openPanel`). Together an MCP session can search a bus headlessly, author a visualiser and open it for the user. Gated by off-by-default `mcp_allow_dashboard_write` and `mcp_allow_ui_control`, wired end-to-end like the existing gates (settings → `mcp::start` → per-permission router → restart on toggle → two `ToggleRow`s in Settings → MCP). [src-tauri/src/mcp/tools.rs](src-tauri/src/mcp/tools.rs), [src/services/mcpBridge.ts](src/services/mcpBridge.ts), [src-tauri/src/settings.rs](src-tauri/src/settings.rs).

- **Agent/MCP-created sessions are first-class in the UI**: backend sessions (including those an MCP agent opens) are adopted into the session store as *known-only* (connected + capabilities, no frame subscription), so an agent-started repeat's Transmit row is fully controllable (start/stop/restart) and session-aware panels can show its source. Adds the `attach_source(session_id, panel)` MCP tool, which opens/focuses a source-aware tab (discovery, decoder, transmit, query, or dashboard) and points it at a session. [src/hooks/useSessionRosterSync.ts](src/hooks/useSessionRosterSync.ts), [src/stores/sessionRoster.ts](src/stores/sessionRoster.ts), [src/stores/transmitRowSession.ts](src/stores/transmitRowSession.ts), [src-tauri/src/mcp/tools.rs](src-tauri/src/mcp/tools.rs).

- **MCP repeat-transmit tools, surfaced in the Transmit queue**: `repeat_transmit_start(session_id, frame_id, data, bus, interval_ms, …)` starts a fixed-interval repeating transmit through a session and returns a `queue_id`; `repeat_transmit_stop(queue_id)` stops it. Both bridge the existing cadence engine that backs the Transmit app's repeat, so an agent can drive a steady periodic frame (e.g. a probe signal) without the UI. An agent-started repeat is mirrored into the Transmit queue as an **Agent**-badged row: the start/stop lifecycle is pushed over the WebSocket transport and handled window-globally, so the row appears (opening the Transmit panel if it is closed) and the human and the agent share one visible, controllable queue — the user can stop or remove it like any other item. Gated by the existing `mcp_allow_control` permission. [src-tauri/src/mcp/tools.rs](src-tauri/src/mcp/tools.rs), [src-tauri/src/mcp/types.rs](src-tauri/src/mcp/types.rs), [src-tauri/src/transmit.rs](src-tauri/src/transmit.rs), [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src/hooks/useRepeatQueueEvents.ts](src/hooks/useRepeatQueueEvents.ts), [src/stores/transmitStore.ts](src/stores/transmitStore.ts).
...
- **wiretap-server writes via COPY instead of a per-row PL/pgSQL function**: the PostgreSQL batch writer now streams each batch through `COPY ... FROM STDIN` (the fastest bulk-insert path) rather than `execute_values()` calling `ingest_can_frame()` once per row. The legacy path remains selectable with `[postgres].write_mode = "function"` for rollback to pre-TimescaleDB schemas. [tools/wiretap-server/wiretap-server.py](tools/wiretap-server/wiretap-server.py), [tools/wiretap-server/wiretap-server.toml](tools/wiretap-server/wiretap-server.toml).

- **Query-app analysis aggregates in SQL instead of fetching raw rows**: mux statistics fetched up to 500k payloads and frequency/gap analysis fetched every timestamp into Rust before aggregating — on a year-long archive that is hundreds of MB over the wire per query. Mux per-byte and 16-bit-word statistics now `GROUP BY` server-side over a `generate_series` lateral (only per-case stats cross the wire), frequency buckets via SQL `LAG` + truncated-epoch grouping (byte-identical bucketing to the old Rust maths, no TimescaleDB required), and gap analysis filters `LAG` deltas against the threshold in SQL with the sort and limit pushed down. `db_frame_inventory` reads the `can_frame_hourly` continuous aggregate when a `to_regclass` probe finds it (full-archive inventory drops from a table scan to milliseconds; the MCP `frame_inventory`/`catalog_coverage` tools inherit this for free), falling back to the raw `GROUP BY` on non-TimescaleDB databases. SQLite capture queries are unchanged. [src-tauri/src/dbquery.rs](src-tauri/src/dbquery.rs).

- **Modbus poll groups are now built in Rust (single source of truth)**: the catalogue → poll-group mapping was duplicated — a TypeScript `buildPollsFromCatalog` for the interactive app and a Rust `build_modbus_polls` for the MCP/headless open flow — which had already drifted (the TS default interval was 1000 ms, the crate's is 5000 ms). The Rust builder is now the single `build_polls_from_catalog` (in `io/modbus_tcp`, built off the `wiretap-catalog` crate's resolved manifest) and is exposed over a new `catalog.polls` WS command; `catalogParser` surfaces the result on the resolved catalogue and the Decoder consumes it. The TypeScript `modbusPollBuilder.ts` is deleted. Behavioural note: a catalogue that sets no interval anywhere now polls at the crate default 5000 ms (was 1000 ms). [src-tauri/src/io/modbus_tcp/mod.rs](src-tauri/src/io/modbus_tcp/mod.rs), [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src/api/catalog.ts](src/api/catalog.ts), [src/utils/catalogParser.ts](src/utils/catalogParser.ts).

- **Modbus catalogues now model slaves as nodes that own the device address**: a Modbus register (formerly mislabelled a "frame") is attributed to a slave **node** (`[node.<name>]` with `device_address`) via a new `node_address = N` key — the register names its slave's address directly and is matched to a node by that address, so renaming a node never breaks a register reference. The device address moved off `[meta.modbus]` (the old single catalog-wide default) and onto each node, so one catalogue can poll several slaves at their own addresses — the per-poll address now flows all the way through to the TCP/RTU drivers (TCP switches slave per poll over one socket via `set_slave`; RTU bakes it into each request) and the MCP/headless open flow. The catalogue editor drops the per-register "Device Address / Use default" field for a **Slave** picker, the sidebar's Nodes view groups registers by slave, the node detail view is protocol-aware (a Modbus node shows its device address and "Registers on this Slave"), and the add/edit-node dialogs gained a device-address field. Register detail/badges now read **"Register"** (one signal) or **"Register Group"** (several). Legacy catalogues with a `[meta.modbus].device_address` and no nodes still load and poll correctly: a slave node is synthesised from the legacy address on parse and the orphaned registers are attached to it. [src-tauri/src/io/modbus_tcp/reader.rs](src-tauri/src/io/modbus_tcp/reader.rs), [src-tauri/src/io/modbus_rtu/reader.rs](src-tauri/src/io/modbus_rtu/reader.rs), [src-tauri/src/mcp/session.rs](src-tauri/src/mcp/session.rs), [src/apps/catalog/views/NodeView.tsx](src/apps/catalog/views/NodeView.tsx), [src/apps/catalog/views/protocol-editors/ModbusConfigSection.tsx](src/apps/catalog/views/protocol-editors/ModbusConfigSection.tsx).

- **The Catalog Editor sidebar is now built from the Rust-resolved catalogue model**: the editor's structure tree was previously produced by a parallel TypeScript parser (`parseTomlToTree`) that re-derived protocol semantics (ids, register numbers, mirror/copy inheritance, mux resolution) and could drift from the canonical `wiretap-catalog` crate. It now builds from the crate's `catalog.parse` output via a new `catalogToTree` mapper, so Rust is the single source of truth. The crate's resolved `Frame` gained the fields the editor needs (authored `key`, `notes`, per-frame `checksums`, `inheritedFields`, serial `delimiter`, mux `default`, a `nodes`/peer list); the ~740-line TS parser is gone (only the raw `tomlParse` accessor remains). Modbus signal rows now show the **synthesised per-signal register** (frame base + bit offset, computed in Rust), and Modbus frame rows show the register as a colour-by-type badge prefix. The protocol badge moved into the sidebar header row beside the collapse toggle. [src/apps/catalog/tree/catalogToTree.ts](src/apps/catalog/tree/catalogToTree.ts), [src/apps/catalog/CatalogEditor.tsx](src/apps/catalog/CatalogEditor.tsx), [src/types/catalogModel.ts](src/types/catalogModel.ts).

- **Catalogue validation is now owned entirely by the `wiretap-catalog` crate**: the editor's form-field validation (CAN id format/range/uniqueness, Modbus register/device-address/type bounds, serial frame-id/delimiter, signal bit bounds + enum/range, checksum algorithm/bounds with negative-index resolution, transmitter-in-peers) plus the catalogue-level serial-encoding-required rule are ported into the crate's `validate` module and exposed as granular `catalog.validate{Meta,Frame,Signal,Checksum}` WS commands. The edit dialogs call these on save (still blocking save on invalid) and the whole-catalogue Validate button is 100% crate-driven. The duplicated TypeScript validator (`validate.ts`, ~500 lines) and the protocol handlers' `validateConfig` methods are removed. [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src/api/catalog.ts](src/api/catalog.ts), [src/apps/catalog/hooks/handlers/](src/apps/catalog/hooks/handlers/).

- **Catalog Editor sidebar is harmonised across protocols, with search and expand/collapse**: the Tree/Frames/Nodes sidebar now reads the same for CAN, Modbus and Serial. The protocol filter badge is pinned to the top above a divider; every row icon is a lucide glyph (the emoji are gone) — protocol frames use the same `Network`/`Server`/`Cable` icons as the badges, signals/mux/checksum/node/meta/config get a consistent set, and the mirror (`Layers`) / copy (`Link2`) indicators are unchanged. Frame IDs render in a single form — `0x000` in hex mode, `0` in decimal — dropping the redundant alternate-base in brackets. An always-visible search box (id or signal name, auto-expanding matches; ⌘F focuses it) and expand-all / collapse-all buttons sit in the header, and the Nodes view groups are collapsible per node. The previously menu-only `FindBar` is reused, now rendered in the sidebar. [src/apps/catalog/layouts/CatalogTreePanel.tsx](src/apps/catalog/layouts/CatalogTreePanel.tsx), [src/apps/catalog/tree/renderTreeNode.tsx](src/apps/catalog/tree/renderTreeNode.tsx), [src/apps/catalog/components/FindBar.tsx](src/apps/catalog/components/FindBar.tsx).

- **Catalog editor edits no longer destroy comments**: UI edits (add/edit/delete a signal, frame, mux, node, checksum or protocol config) are now applied in Rust by the `wiretap-catalog` crate using `toml_edit`, which edits the document in place — so a comment-rich catalogue keeps every `#` comment and its formatting when you add a single entry and save. Previously each edit round-tripped the whole document through a comment-less TOML serialiser, stripping all comments on the first change. The editor's TypeScript ops became thin async wrappers over a new `catalog.edit` command (`EditOp` primitives: set-table, upsert-frame, upsert-array-item in sorted position, rename-key, delete); the old `tomlStringify`/`__HEX__` serialiser is gone. New signals/frames insert in sorted position so existing entries and their comments don't move; masks render as hex literals natively. [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src/apps/catalog/editorOps.ts](src/apps/catalog/editorOps.ts).

- **The "Graph" app is now "Dashboard", with an analog-gauge icon**: a full rename — the app id, label, native-menu entry, launcher/top-bar icon (now lucide `Gauge`), and the Decoder/Discovery "send to" actions, plus all internals: the `graph/` source directory → `dashboard/`, the store and types (`useDashboardStore`, `DashboardPanel`, `dashboardStore.ts`), the views (`DashboardGrid`, `DashboardTopBar`), the saved-layout feature (`DashboardLayout`, `dashboardLayouts.ts`, the settings "Dashboard Layouts" section), and the i18n namespace (`dashboard.json`). **Persisted keys are deliberately kept on their original `graph.*` names to avoid a data migration**: the Tauri-store keys `graph.layouts` (named layouts) and `graph.lastSession` (autosave), and the `graph_buffer_size` setting field — these stay `graph*` and a one-time copy-migration would be needed to rename them. A legacy `graph` Dockview component alias keeps previously-saved window layouts loading. [src/apps/dashboard/](src/apps/dashboard/), [src/apps/registry.ts](src/apps/registry.ts), [src/components/MainLayout.tsx](src/components/MainLayout.tsx).
...
- **MCP catalog write tools, behind two catalog-specific permissions**: an agent can now persist decode work, not just read it. `validate_catalog` (always available) parses + validates TOML and returns `{ valid, errors }`; `create_catalog` writes a *new* catalogue file (refuses if it exists); `update_catalog` overwrites an *existing* one (refuses if it doesn't). Both validate before writing and reject — without touching disk — on any finding, take the full TOML (preserving comments, mux shorthands and mirror inheritance), and sanitise the filename under the decoder directory. Two new off-by-default settings gate them independently of the control gate — `mcp_allow_catalog_write` (create) and `mcp_allow_catalog_modify` (overwrite) — so a user can grant creating new catalogues without granting overwriting existing ones; with neither granted only `validate_catalog` is exposed. Wired end-to-end like the existing MCP gates (Rust `AppSettings` → `mcp::start` → one router per permission → restart on toggle → two `ToggleRow`s in Settings → MCP). [src-tauri/src/mcp/tools.rs](src-tauri/src/mcp/tools.rs), [src-tauri/src/settings.rs](src-tauri/src/settings.rs), [src/apps/settings/views/McpServerView.tsx](src/apps/settings/views/McpServerView.tsx), [docs/mcp-analysis-tools.md](docs/mcp-analysis-tools.md).

- **Headless MCP analysis tools for captures and databases**: ten new read-only MCP tools let an agent inspect recorded CAN data with no Decoder/Discovery view open — each takes either a `capture_id` (SQLite) or a `profile_id` (PostgreSQL) and reads the store directly. `frame_inventory` rolls up every frame id (count, first/last, dlc); `frame_byte_profile` classifies each payload byte of a frame as static/counter/sensor over a sample (the headless Rust equivalent of the Discovery byte analysis); and `catalog_coverage` diffs a decoder catalogue against the data — present/missing frames, uncatalogued ids, and a high/medium/low/unset signal-confidence rollup (mirror-inherited signals excluded so each definition counts once). The seven Query-app engines (`query_byte_changes`, `query_frame_changes`, `query_distribution`, `query_gap_analysis`, `query_frequency`, `query_first_last`, `query_mux_statistics`) are also exposed, dispatching to the same postgres/capture backends. Byte sampling reads the most-recent N frames (so it reflects current behaviour, not the start of a multi-month archive), and `catalog_coverage`'s per-frame byte roles are opt-in (`include_byte_roles`) to keep the headline diff a single cheap aggregation. New module [src-tauri/src/analysis.rs](src-tauri/src/analysis.rs); backends extended in [src-tauri/src/dbquery.rs](src-tauri/src/dbquery.rs) and [src-tauri/src/capture_db.rs](src-tauri/src/capture_db.rs); tools wired in [src-tauri/src/mcp/tools.rs](src-tauri/src/mcp/tools.rs); reference [docs/mcp-analysis-tools.md](docs/mcp-analysis-tools.md).
...
- **MCP settings copy is now client-generic ("MCP client", not "Claude client")**: the MCP server isn't Claude-specific, so the Settings → MCP descriptions and code comments now say "MCP client" throughout. The connection helper is relabelled "Connect an MCP client" with the concrete `claude mcp add …` command shown under an "Example for Claude Code:" caption. [src/apps/settings/views/McpServerView.tsx](src/apps/settings/views/McpServerView.tsx).
...
- **Repeat-transmit and Modbus polling share one cadence primitive**: three places hand-rolled the identical interval loop — the Transmit app's repeat tasks (`io_start_repeat_transmit` and its serial/group variants), the multi-source Modbus poll task, and the standalone Modbus TCP reader — each spelling out *fire immediately, then tick every N ms, stop on a cancel flag, skip ticks while paused*. That timing triad is now a single `Cadence` type ([src-tauri/src/io/periodic.rs](src-tauri/src/io/periodic.rs)); callers keep their own per-tick body (a transmit logs a `TransmitResult`; a poll emits a `FrameMessage` into the rx stream) and just drive it with `while cadence.next().await.is_some() { … }`. No behaviour change — pacing, cancel and pause semantics are identical — it removes ~130 lines of duplicated loop scaffolding and gives the two mechanisms one source of truth. Modbus RTU is untouched: its half-duplex bus needs strictly-ordered request/response, which a per-task interval can't model. [src-tauri/src/transmit.rs](src-tauri/src/transmit.rs), [src-tauri/src/io/broker/spawner.rs](src-tauri/src/io/broker/spawner.rs), [src-tauri/src/io/modbus_tcp/reader.rs](src-tauri/src/io/modbus_tcp/reader.rs), [docs/session-flow.md](docs/session-flow.md).

- **Modbus folded into the Decoder — one signal-decoding app**: the standalone Modbus app and the Decoder did the same job (catalogue-driven signal decoding over an IO session) — the only real difference was Modbus *polling* registers, which didn't warrant a separate app. The Modbus app is **removed** and its behaviour absorbed by the **Decoder**: load a Modbus catalogue (or pick a Modbus profile whose `preferred_catalog` is one) and the Decoder builds the poll groups, injects them into the session, shows pause/resume **poll controls** + a poll/register-count badge in the top bar, and renders each register group through the normal signal view — a Modbus *frame* is a register block (often many signals, e.g. the Sungrow Battery block), so it maps onto the existing FrameCard exactly like a CAN frame, gaining a register-type badge (`holding`/`input`/`coil`/`discrete`) and the poll interval. Poll groups are now built **before** the watch starts — an awaited `onBeforeStart` in the IO picker pre-loads the catalogue — so a Modbus session is created *with* polls in a single connection instead of starting pollless and reconnecting; on single-connection devices the old reconnect spawned a rival connection that broke the device's one TCP slot (every read failed), so this is also a fix. The `modbus` launcher entry, store, views, `utils/modbusRegisters.ts` and the `modbus` i18n namespace are deleted; the per-source poll **stagger/queue** is unchanged and the Rust Modbus *protocol* and *scanning* code is untouched. [src/apps/decoder/Decoder.tsx](src/apps/decoder/Decoder.tsx), [src/apps/decoder/views/DecoderTopBar.tsx](src/apps/decoder/views/DecoderTopBar.tsx), [src/apps/decoder/views/DecoderFramesView.tsx](src/apps/decoder/views/DecoderFramesView.tsx), [src/stores/decoderStore.ts](src/stores/decoderStore.ts), [src/hooks/useIOSourcePickerHandlers.ts](src/hooks/useIOSourcePickerHandlers.ts), [src/apps/registry.ts](src/apps/registry.ts), [src/apps/apps.json](src/apps/apps.json), [docs/session-flow.md](docs/session-flow.md).

- **Exact decoded values — scaling moved to `Decimal`, in a shared `wiretap-decode` crate**: live decode for every protocol (CAN, Serial, Modbus) runs through `decode_by_id` in [`wiretap-catalog`](https://github.com/Wired-Square/wiretap-lib-rs), which scaled `raw × factor + offset` in `f64` and approximated the display with a trim heuristic — so a decoded value could still read `55.50000000000001`. The [`wiretap-lib`](https://github.com/Wired-Square/wiretap-lib-rs) workspace gained a new **`wiretap-decode`** crate holding the protocol-agnostic numeric core (bit extraction, 16-bit word-swap, `Endianness`, exact `Decimal` scaling, value formatting); `wiretap-catalog` now builds its general decode path *and* its Modbus poll path on that one `scale()` — the same exact-`Decimal` treatment the Modbus manifest already used (`3374 × 0.1` is `337.4`, not `337.40000000000003`). The `DecodedSignals` wire shape is unchanged (`scaled` stays a number for the Graph; `display` is the now-exact string every value table renders), so this is no frontend change — decoded displays are simply correct. WireTAP pins **`wiretap-catalog` v0.7.1** (the workspace is now versioned and released as a unit). [src-tauri/Cargo.toml](src-tauri/Cargo.toml).

- **Catalogue parsing fully on Rust too — the TS parse engine is gone**: the catalogue migration below moved *decode* to the [`wiretap-catalog`](https://github.com/Wired-Square/wiretap-lib-rs) crate but left a ~430-line TypeScript TOML parser (`catalogParser.ts`'s `parseCatalogText` + mirror/copy resolution + mux/signal/config parsing) feeding the Decoder, Graph and Query in-memory models — a second parser that could drift from the Rust one. `loadCatalog` now calls the `catalog.parse` WS command (the same Rust crate path Modbus already used) and runs the result through a thin **adapter** that maps the resolved `Catalog` (camelCase, `inherited` flag) onto the legacy `ParsedCatalog` shape (snake_case, `_inherited`) the consumers expect — recursing through nested mux and re-deriving the serial header byte-positions (`frame_id_*`, `source_address_*`, `header_fields[]`) that the crate stores only as raw masks. All three apps converge through `loadCatalog` in one move, and downstream consumers (the Decoder's mirror validation, `frameExport`, `sessionConfigMerge`, `api/io`, Discovery's framed view) keep their existing `ParsedCatalog`/`SerialFrameConfig` interface unchanged. The file drops from ~830 to ~250 lines — the parser is deleted; only the adapter, the `Catalog`→`ParsedCatalog` types, and the `parseCanId`/`findFrameByNumericId` id utilities remain. Verified in-app against serial header-field extraction (Tesla Gen2 HPWC), CAN nested mux, and the Graph. [src/utils/catalogParser.ts](src/utils/catalogParser.ts), [src/stores/decoderStore.ts](src/stores/decoderStore.ts), [src/stores/graphStore.ts](src/stores/graphStore.ts), [src/apps/query/Query.tsx](src/apps/query/Query.tsx), [docs/session-flow.md](docs/session-flow.md).

- **Serial header byte-positions derived once, in Rust**: the TS adapter above still re-derived the serial convenience fields (`frame_id_start_byte/bytes/byte_order`, `source_address_*`, `header_fields[]`) from raw masks via its own `maskToBytePosition` — duplicating the mask→byte logic the [`wiretap-catalog`](https://github.com/Wired-Square/wiretap-lib-rs) crate already runs at decode time. The crate (**v0.6.0**) now computes these at parse time and serialises them on `SerialConfig`, so `adaptSerialConfig` simply renames them and `maskToBytePosition` is deleted. One derivation, shared by Rust decode and the frontend; no behaviour change (serial header badges + frame-id/source-address extraction render as before). [src/utils/catalogParser.ts](src/utils/catalogParser.ts), [src/types/catalogModel.ts](src/types/catalogModel.ts), [docs/session-flow.md](docs/session-flow.md).

- **Catalogue loaded once per selection in Decoder/Graph, not twice**: selecting a catalogue parsed the same TOML twice over the WebSocket — once via `loadCatalog` (`catalog.parse`, to build the UI model) and again via a separate `catalog.attach` effect (to bind Rust decode). `catalog.attach` now **returns the resolved Catalogue**, so a new `attachAndResolve` helper binds decode *and* yields the model from a single parse. The Decoder and Graph collapse their two catalogue effects into a parse-free **mirror** effect (tracks the session's path) plus one **load/attach** effect (`loadCatalogForSession` when a session exists — falling back to a model-only load if attach fails — else a plain `loadCatalog`); each store grew an `applyParsedCatalog`/`setCatalogPath` split so both paths feed one model builder, and the now-redundant `handleCatalogChange` plumbing is removed. Modbus keeps its existing flow (its catalogue load is coupled to a poll-reconnect that reinitialises the session). [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src/api/catalog.ts](src/api/catalog.ts), [src/utils/catalogParser.ts](src/utils/catalogParser.ts), [src/stores/decoderStore.ts](src/stores/decoderStore.ts), [src/stores/graphStore.ts](src/stores/graphStore.ts), [src/apps/decoder/Decoder.tsx](src/apps/decoder/Decoder.tsx), [src/apps/graph/Graph.tsx](src/apps/graph/Graph.tsx).

### Fixed

- **Decoder: selecting a serial catalogue mid-stream now reframes the source live**: serial framing (SLIP/COBS/…) is applied by the backend read loop, so connecting first and then picking a serial catalogue left the source in `Raw` mode — it streamed raw bytes that were never framed, so nothing decoded (you had to pick the catalogue before connecting). A new **`io_set_framing`** session command swaps the running serial source's framer **in place** — same session, the device stays open and the attached catalogue keeps decoding (no reconnect, no re-attach). It mirrors the transmit plumbing: a per-source control channel (`SourceMessage::ControlReady`) the read loop drains in its non-blocking window and swaps the framer + frame-id config; the broker records a framing override so re-broadcast capabilities flip `rx_frames` true, and creates a frame capture on the fly (a bytes-only session has none) so the now-framed messages are stored, streamed and decoded. The Decoder calls it when the catalogue's `encoding` first appears or changes while streaming (falling back to a full re-watch if the live swap fails); frame-id-only changes within the same framing still use the lighter clear-and-re-extract path. [src-tauri/src/io/serial/reader.rs](src-tauri/src/io/serial/reader.rs), [src-tauri/src/io/broker/mod.rs](src-tauri/src/io/broker/mod.rs), [src-tauri/src/io/mod.rs](src-tauri/src/io/mod.rs), [src-tauri/src/transmit.rs](src-tauri/src/transmit.rs), [src/api/io.ts](src/api/io.ts), [src/apps/decoder/Decoder.tsx](src/apps/decoder/Decoder.tsx).

### Added

- **Per-panel Frame ID format toggle — flip hex/dec without touching settings**: frame IDs followed a single global setting (`display_frame_id_format`), so comparing one panel in hex against another in decimal meant changing the setting for everything. Each frame-aware app top bar now carries a **"Flip" control** that cycles the panel's frame-id base **Auto → Dec → Hex → Auto** — *Auto* follows the global default (and shows it, e.g. `Auto · HEX`); *Dec*/*Hex* override that panel only, leaving every other panel and the global setting untouched. The override is ephemeral per panel instance. Under the hood the wire-to-screen path is centralised: the raw `frame_id: u32` + `is_extended` still travel untouched from Rust through the binary WebSocket (formatting stays a presentation concern — nothing new in Rust), and a new `FrameIdFormatProvider`/`useFrameIdFormat` context resolves the *effective* format once per panel, which the shared `FrameDataTable`/`FramePicker` and each app's render leaves consume; apps opt in with a one-line `withFrameIdFormat(Inner)` wrapper. Live in Discovery, Decoder, Catalog Editor, Transmit, Modbus and Graph (Graph's signal-picker/candidate/hypothesis/panel-config dialogs previously hard-coded hex — they now follow the panel format too, and their frame-id dropdowns show just the formatted id, dropping the redundant `(decimal)` suffix). [src/hooks/useFrameIdFormat.tsx](src/hooks/useFrameIdFormat.tsx), [src/components/FrameIdFormatToggle.tsx](src/components/FrameIdFormatToggle.tsx), [src/components/AppTopBar.tsx](src/components/AppTopBar.tsx).

- **Data Source picker: tabbed, with a one-step decoder**: the picker was one long scroll — active sessions, captures, external (CSV), recorded DB sources and live devices all stacked — and a decoder still had to be chosen separately in each app's top bar after connecting. It's now organised into **Sessions / Captures / Devices** tabs (the strip stays pinned while the list scrolls): *Sessions* lists joinable live/capture sessions and is the default tab when one exists; *Captures* holds local SQLite captures, recorded DB sources (e.g. PostgreSQL) and CSV import; *Devices* lists the live interfaces. The dialog opens on the tab matching the current selection. A new **Decoder footer** attaches a catalogue *as the session is created* — the chosen path rides through `LoadOptions.catalogPath` and `useIOSessionManager` sets it on the new session via `setSessionCatalogPath` (the cross-app channel `useSessionCatalog` already mirrors), so any decode-aware app on that session binds it with no extra step. The footer **auto-fills from the source's `preferred_catalog`** (until you pick or clear one by hand), renders the selected decoder as a source-style row, and — when the catalogue declares **serial framing** (e.g. SLIP) — parses it and reflects that encoding in the source's framing dropdown so the framing is explicit before you connect. Decoder and Graph mirror the picker's choice into their own catalogue state; the other apps that use the picker are unaffected. A shared `findCatalogByPath`/`normalisePath` in [catalogUtils.ts](src/utils/catalogUtils.ts) replaces three hand-rolled copies. [src/dialogs/IoSourcePickerDialog.tsx](src/dialogs/IoSourcePickerDialog.tsx), [src/dialogs/io-source-picker/SourceList.tsx](src/dialogs/io-source-picker/SourceList.tsx), [src/dialogs/io-source-picker/SourceTabs.tsx](src/dialogs/io-source-picker/SourceTabs.tsx), [src/dialogs/io-source-picker/DecoderPicker.tsx](src/dialogs/io-source-picker/DecoderPicker.tsx), [src/hooks/useIOSessionManager.ts](src/hooks/useIOSessionManager.ts), [src/hooks/useIOSourcePickerHandlers.ts](src/hooks/useIOSourcePickerHandlers.ts), [docs/session-flow.md](docs/session-flow.md).

- **Catalogue decoding moved to Rust — one shared library, decoded once on the wire**: Catalogue handling was previously split three ways — a Rust validator/DBC bridge in `src-tauri`, a full TypeScript parser (`catalogParser.ts`) *and* TypeScript decode engine (`signalDecode.ts`/`bits.ts`/`muxCaseMatch.ts`) that re-decoded every frame on the UI thread, and a separate editor serialise path. The shared [`wiretap-catalog`](https://github.com/Wired-Square/wiretap-lib-rs) crate (v0.5.0) is now the **single source of truth** for reading, validating, decoding, encoding and DBC-converting catalogues across **CAN, Serial and Modbus**.

  WireTAP drives it over the existing binary WebSocket two ways:
  - **Request/response** `catalog.*` commands (`parse` / `validate` / `import_dbc` / `export_dbc`) — the Catalog Editor's validation, DBC import and DBC export now route here.
  - **Decode-on-attach push stream**: `catalog.attach { session_id, content }` binds a catalogue to a session; the backend then decodes each frame batch **once, in Rust** (`decode_by_id` — `frame_id_mask`, signals, mux, header fields, source address) and pushes a new `DecodedSignals` (0x14) message alongside the raw `FrameData`. The **Decoder, Graph and Modbus** apps consume this instead of decoding in TypeScript; raw `FrameData` keeps flowing for Discovery/Analysis/raw-hex/Calculator. Attachments auto-detach on unsubscribe.

  This removes the duplicate decode (fixing, e.g., a Graph bug that read Modbus registers little-endian — a temperature showing -2867.2 instead of 14.4 — because the two engines disagreed), and lifts per-frame decoding off the UI thread. The migration deleted **~4,000 lines** of now-duplicated code: the TS decode engine (`signalDecode.ts`, the decoder's TS decode), and the Rust `dbc_import.rs`/`dbc_export.rs`, `validate_catalog`, `parse_modbus_catalog` and unused doc structs. The Decoder keeps its mirror-validation byte-compare on the raw path; the Catalog Editor's signal pickers still use the TS `catalogParser` parser (decode itself is all Rust now). [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs), [src-tauri/src/ws/dispatch.rs](src-tauri/src/ws/dispatch.rs), [src-tauri/src/ws/protocol.rs](src-tauri/src/ws/protocol.rs), [src/services/wsProtocol.ts](src/services/wsProtocol.ts), [src/stores/sessionStore.ts](src/stores/sessionStore.ts), [src/stores/decoderStore.ts](src/stores/decoderStore.ts), [src/apps/graph/Graph.tsx](src/apps/graph/Graph.tsx), [src/apps/modbus/Modbus.tsx](src/apps/modbus/Modbus.tsx), [docs/session-flow.md](docs/session-flow.md).

- **Modbus catalogue shorthands: name a frame by its register, and signal-less registers**: Authoring a Modbus catalogue no longer requires a `register_number` field or a `[[signals]]` block for every register. A frame can be **named by its register** — `[frame.modbus.0x32F9]` or `[frame.modbus.13049]` — and a register that *is* a single value can carry its decoding fields (`format`/`factor`/`offset`/`unit`/`signed`/`enum`) at the frame level, synthesising one full-width signal. Both shorthands are resolved by a new **shared Rust crate, [`wiretap-catalog`](https://github.com/Wired-Square/wiretap-lib-rs)** (extracted from the ESS add-on so the two projects share one parser/decoder), exposed to the frontend via a `parse_modbus_catalog` command — so the Modbus app builds its poll groups and decodes from the resolved model. The **Catalog Editor** supports authoring them: the frame name is honoured as the TOML key (renaming to `2581` no longer becomes `register_2581`), the Register Number field is optional, and a warning appears (inline and on save) when a non-numeric name has no register number. [src/apps/modbus/stores/modbusStore.ts](src/apps/modbus/stores/modbusStore.ts), [src/api/catalog.ts](src/api/catalog.ts), [src/apps/catalog/protocols/modbus.ts](src/apps/catalog/protocols/modbus.ts), [src/apps/catalog/editorOps.ts](src/apps/catalog/editorOps.ts), [src-tauri/src/catalog.rs](src-tauri/src/catalog.rs).

- **Catalog Editor: view-mode selector and protocol-filter badges**: The tree panel had a single hierarchical view plus a funnel button that filtered frames to one transmitter — awkward for finding a frame by protocol or owner in a large catalog. The funnel is replaced by a **Tree / Frames / Nodes** segmented selector: *Tree* is the existing hierarchy, *Frames* is a flat register-sorted list of every frame, and *Nodes* groups frames by the node that owns them (CAN `transmitter`, or Modbus `device_address` shown as "Slave N", with an "Unassigned" bucket). Non-Tree modes reuse the existing row renderer, so labels, selection, and expand-to-signals still work. Separately, the **CAN / Modbus / Serial badges** at the top of the panel are now **protocol-filter toggles** — clicking one narrows every view to that protocol's frames (the meta and node sections stay visible); the active badge is ringed and re-clicking clears it. Catalog configuration moved entirely to the toolbar's Settings button, and a badge's `!` (missing protocol config) now carries its own explanatory tooltip. Grouping/filtering live in a new [frameGroups.ts](src/apps/catalog/tree/frameGroups.ts) helper (`buildFrameGroups`, `applyProtocolFilter`); the old transmitter filter (`shouldShowNode`) is removed. [src/apps/catalog/layouts/CatalogTreePanel.tsx](src/apps/catalog/layouts/CatalogTreePanel.tsx), [src/apps/catalog/CatalogEditor.tsx](src/apps/catalog/CatalogEditor.tsx).

- **Decoder shows each Modbus signal's own register**: a Modbus catalogue frame groups several signals into one polled register block, so a multi-register value's actual address wasn't visible per signal. The shared [`wiretap-catalog`](https://github.com/Wired-Square/wiretap-lib-rs) crate (**v0.7.2**) now synthesises each signal's `modbusRegister` and `modbusRegisterCount` at parse time — word registers (holding/input) add `start_bit / 16` to the frame's base register and span `⌈bit_length / 16⌉` registers; coils/discretes add `start_bit` and span `bit_length` — and the Decoder renders it next to the signal name, e.g. `Battery_Voltage 13019` or a span `Total_PV_Battery_Generation 5003–5004` (in the panel's frame-id base). Verified against a live Sungrow SH10RS by reading the registers directly over the MCP server and decoding independently. WireTAP pins **`wiretap-catalog` v0.7.2**. [src/apps/decoder/views/DecoderFramesView.tsx](src/apps/decoder/views/DecoderFramesView.tsx), [src/utils/catalogParser.ts](src/utils/catalogParser.ts), [src/types/catalogModel.ts](src/types/catalogModel.ts), [src/types/decoder.ts](src/types/decoder.ts), [src-tauri/Cargo.toml](src-tauri/Cargo.toml).

- **Modbus → Registers: Raw column hex/ASCII toggle**: The register table's Raw column was hardcoded to space-separated hex, so string-valued registers (firmware/version strings like `ARM_SUNSTONE-H_V11_V01_A`) were only readable in the decoded Signals column, never the raw bytes. A new icon toggle in the Registers-tab header (next to the time-format switch) flips the Raw column between hex and **ASCII** — printable bytes render between pipes (`|…|`), non-printable as `.`, via the shared [`bytesToAscii`](src/utils/byteUtils.ts) helper that Discovery and the byte view already use. The choice is session-local (resets to hex on restart), mirroring the existing `timeFormat` toggle pattern, and only appears on the Registers tab. [src/apps/modbus/views/ModbusRegistersView.tsx](src/apps/modbus/views/ModbusRegistersView.tsx), [src/apps/modbus/Modbus.tsx](src/apps/modbus/Modbus.tsx).

- **Catalog Editor: Modbus frames show their register address in the tree**: Modbus frames were labelled by their TOML table name only (`📟 ems_control`), so a frame couldn't be found by the register number it polls — the `register_number` was in the node metadata but never displayed. The tree now appends the formatted address and register type, e.g. `ems_control (0x32F9) [holding]`, reusing the shared [`formatFrameId`](src/utils/frameIds.ts) so the address tracks the **Settings → Display** hex/decimal preference (showing `0x32F9` or `13049`) and matches how the Modbus app itself renders the same register. [src/apps/catalog/tree/renderTreeNode.tsx](src/apps/catalog/tree/renderTreeNode.tsx), [src/apps/catalog/CatalogEditor.tsx](src/apps/catalog/CatalogEditor.tsx).

- **MCP server for live runtime introspection (opt-in)**: WireTAP can now host a local [Model Context Protocol](https://modelcontextprotocol.io) server so an external AI client (e.g. Claude Code) can query live app state — active sessions, captures and their frames, decoder catalogs (TOML), IO profiles, playback position, recent logs, and live one-off Modbus register reads — and, behind separate opt-in gates, drive the app (transmit a frame, write Modbus registers/coils, replay a capture) or manage session lifecycle (open a session — building Modbus poll groups from the profile's catalog so it polls immediately — and stop one). MCP client connections, and sessions opened/torn down via MCP, appear in the Session Manager log. The richest reverse-engineering data (per-byte payload analysis, decoded signals, the live discovery frame map) is fetched from the frontend over the existing binary WebSocket channel via a new reverse request/response path. The server speaks streamable-HTTP on `127.0.0.1` at a configurable port (default 8787) with an optional bearer token, and is governed by two independent, off-by-default switches in **Settings → MCP Server**: one to enable the server, one to allow the control tools. Read-only and localhost-bound by default.
...
- **Serial → terminal tidy-up: clearer font, clipboard support, live font-size stepper**: The xterm.js terminal in the Serial app was cramped and had no way to get text in or out — xterm renders to a canvas, so the browser's native copy/paste can't see the selection. The display now uses **JetBrains Mono** (larger x-height and more open letterforms than the previous Ubuntu Mono, far easier to read) with breathing room (`lineHeight 1.25` + slight `letterSpacing`), the selection has a **visible highlight** (a `selectionBackground` was added to the otherwise near-black theme so dragging to select is actually perceptible), and clipboard is wired end-to-end: **highlighting copies** (copy-on-select via `onSelectionChange`), backed by a **right-click context menu** (Copy — shown only when there's a selection — Paste, Select all, Copy all) plus **keyboard shortcuts** via xterm's `attachCustomKeyEventHandler`. The shortcuts are platform-correct — `Cmd+C/V/A` on macOS, `Ctrl+Shift+C/V/A` on Windows/Linux — so a bare `Ctrl+C` still passes through to the device as an interrupt rather than being hijacked for copy. Paste routes through `term.paste()` so it lands in the view via the same input path as typing, with local-echo and CRLF handling still applied. The top nav gains a **Copy all** button (dumps the full scrollback via a new `getText()` on the terminal handle, and flashes a green "Copied" tick on success) and a **− / size / +** font-size stepper that zooms the terminal live, clamped to 8–24px. Font size is runtime-only state on `serialStore` (resets to 13px on restart), mirroring the local-echo toggle. Clipboard work routes through a shared `src/api/clipboard.ts` wrapper over the Tauri clipboard-manager plugin (see the paste/selection fix below). There is no "cut": terminal output isn't editable.
- **Serial → terminal: paste no longer prompts or double-pastes, and the text cursor shows over the terminal**: Pasting used the browser `navigator.clipboard.readText()` API, which in the macOS WKWebView pops the OS paste-authorisation dialog on every paste. Clipboard reads/writes now go through the native **Tauri clipboard-manager plugin** behind a single `src/api/clipboard.ts` wrapper (also adopted by the Frame Calculator), so `Cmd+V` and the context-menu Paste land directly with no prompt. The paste/copy shortcuts now also `preventDefault()` on the handled keydown — xterm listens for the native `paste` event too, so without it `Cmd+V` pasted twice (once from our handler, once natively). Separately, the global `* { cursor: default }` reset was overriding xterm's own `cursor: text` on every descendant (incl. the row spans), so the terminal only ever showed the arrow and mouse selection felt unavailable — a scoped `.xterm, .xterm *` rule restores the I-beam, making selection discoverable (xterm already handled the drag-selection internally).

## [0.6.5] - 2026-05-13

### Added

- **Rules → Bridge filter dialog with kind-aware labels and mask calculator**: The Add Bridge dialog rendered two unlabelled hex inputs after the Mask/Range selector, leaving users guessing which value went where or what the mask bits meant. The dialog now switches labels per filter kind (**CAN ID / Mask** for mask mode, **Lo / Hi** for range mode), a `?`-toggled inline help block sits beside each row, and Mask mode gets an **interactive bit-level calculator** that lets users experiment with ID + mask values and see which IDs match before committing the rule. Range mode shows prose-only help since Lo–Hi is self-evident; the calculator is reserved for Mask mode where "0 = don't care" is the recurring point of confusion. The bit-row primitive is reusable across any mask-explanation UI.

- **Rules → Bridge: range filters, IDE-flag matching, and per-bridge default action**: Mirrors the bridge protocol expansion shipped in framelink. The Add Bridge dialog now exposes per-filter **Mask/Range** and **Std/Ext/Any** selectors plus a bridge-level **Default Action** — *pass* forwards everything except the listed IDs, *block* forwards only the listed IDs. The filter-section label and tooltip switch between "Frames to drop" and "Frames to forward" depending on the selected default, keeping the interaction one-screen rather than forcing the user to mentally invert filter semantics. Bridge and bridge-filter descriptors carry the typed framelink protocol enums end-to-end; the Rust-side parser reads them by name with sensible defaults so a missing field isn't a hard error. Every label, select, and per-row input gets a tooltip explaining what the field controls in language a mildly-technical CAN user can act on.
...
- **SMP/OTA: WebSocket-pushed event stream and unified Firmware tab over framelink**: The legacy Tauri SMP commands are replaced by a thin shim over framelink's OTA event stream. Each event is pushed to all WebSocket clients via a new global message — new WS commands cover listing slot images, starting an OTA, and cancelling one. Only one OTA may be in flight at a time, enforced by an in-flight guard installed at swap that drops on every exit path; cancel is driven by an async notify. Device IDs resolve through the framelink discovery handle, so either a BLE local name or a fully-qualified device id is accepted, and starting an OTA pre-emptively drops any cached session so the new exchange opens a fresh SMP link. The split BLE/UDP upgrade flow collapses into a single Firmware tab driven entirely by the event stream — slots render as cards, the file picker is always visible, the event log sits under the progress bar (capped at 200 entries), and upload progress arrives as a single combined state update per event. The legacy Rust upgrade module is removed (~340 lines) along with its Zustand upgrade store (~175 lines). UDP-only for now — BLE OTA stays disabled in the device view pending investigation. Tracing from framelink is routed to stderr behind the standard log-level env var so debug traces surface without a custom log sink.

- **Devices → SMP-UDP connect keyed by mDNS device id**: The previous UDP connect path reverse-looked-up the framelink device id from the discovery snapshot, racing against the snapshot's own update cadence — the first attempt typically failed with "no mDNS device matches IP:PORT". Replaced with a connect call that takes the device id directly; the frontend already has it on the per-transport device record and now plumbs it through to the devices store. Post-reboot reconnect, previously a polling loop, is now a one-shot subscription: as soon as device discovery re-adds the rebooted device's UDP entry, the UDP connect fires once and resolves. The device-disappeared listener is wired so framelink's Lost events actually clear the store.

- **Devices → rescan signal, post-reboot BLE reconnect, BLE-attach drop**: Post-PR follow-ups to the framelink discovery migration. The device scan now emits a "scan finished" event 8 s after start so the frontend's Rescan button lights up — the new discovery handle scans forever, so this is the UI "settling" signal; the forwarder, heartbeat, and settled-signal tasks all abort cleanly on stop so hitting Rescan re-arms a fresh window. New "reconnect BLE by name" command: after a firmware test-boot the device's BLE peripheral identifier rotates, so the cached id from before the upload is stale ("device id not currently known to discovery"). The local name (e.g. *WiredFlexLink-4CD4*) is firmware-stable, so **Reconnect & Confirm** now polls discovery by name and retries the SMP-BLE open until a fresh BLE+SMP match connects. The "attach SMP to an existing BLE peripheral" hack and its frontend wrapper are dropped — that path worked around the BLE library's single-owner constraint, but framelink's discovery now refcounts a peripheral lease, so the second open on the same device just clones the live lease.

### Fixed

- **Devices → WiFi provisioning reported failure on a successful provision**: The WiFi tab overwrote the connected state set by the BLE-provision listener with a "waiting" state once the provision call resolved, and a 30 s timer then flipped that to a timed-out error. The provision call now resolves only after the device has reached a terminal state, so the post-resolve fallback just commits the connected state if the listener didn't beat us to it; the unreachable waiting state, its timer, and the orphan locale strings are removed. Related polish: initial-tab selection now prefers WiFi provisioning over SMP on BLE Connect, since BLE OTA is disabled and WiFi provisioning is the only operation currently performable over BLE.

- **Firmware tab UI**: Non-Active slot badges no longer render dimmed — the "Confirmed" badge previously looked like an unset state. The upload-progress counter is rounded to whole KB and padded to total's width inside a monospace container so it stops shifting layout. A **Confirm Image** button surfaces next to Select Firmware whenever an active-but-unconfirmed slot exists, as a manual fallback when the automatic reconnect-and-confirm flow didn't complete.

## [0.6.4] - 2026-05-03

### Added

- **Serial → STM32 DFU flasher: live wire-up + unified device picker**: The DFU tab's stub backend is replaced with a real `dfu-nusb`-driven implementation in [src-tauri/src/flashers/dfu_flasher.rs](src-tauri/src/flashers/dfu_flasher.rs). `list_devices` now walks `nusb::list_devices()` filtered by interface class 0xFE/subclass 0x01 (DFU class) — captures STM32 bootloaders (0483:DF11) and any other DFU-class device, falling back to a synthetic `bus<N>-dev<M>` identifier when the device reports no USB serial. `flash` opens by USB serial, claims the first DFU-class interface, calls `DfuNusb::open` → `into_sync_dfu` → `override_address(addr)` → `download(...)`, with a progress closure that emits `flasher-progress` per chunk and triggers `nusb::Device::reset()` on a cloned handle to abort cleanly when the user cancels. After a successful download, best-effort `detach()` + 50 ms delay + `usb_reset()` exits DFU mode and starts the new firmware. Image parsing supports `.bin` (raw), Intel `.hex` (first contiguous span via the `ihex` crate), and `.dfu` (strips the 16-byte DFU suffix; if the DfuSe magic is present, extracts the first target's first element's image data — covers the common single-image case for STM32CubeProgrammer output). `dfu-core` auto-detects DfuSe from the bootloader's interface alt-string memory layout, so SET_ADDRESS_POINTER + ERASE_PAGE on STM32 internal flash regions works without any explicit branching here. Cargo.toml gains a `nusb_dfu = { package = "nusb", version = "0.1" }` alias because `dfu-nusb` 0.1 pins `nusb` 0.1 while `gs_usb` already uses `nusb` 0.2's `MaybeFuture` API; the two versions coexist via the alias. New unit tests cover the DFU suffix parser, DfuSe element extraction, and Intel HEX first-span coalescing.

  **Device selection moved from the DFU view into the shared SerialPortPicker** in the top nav ([src/components/SerialPortPicker.tsx](src/components/SerialPortPicker.tsx)) — one picker drives all four Serial tabs. The popover gains a "USB DFU" section below the serial-port list (always visible regardless of active tab so the user can pre-select a DFU device while still on a serial tab); a `mode: "serial" | "dfu"` prop drives whether the picker button shows the selected serial port or the selected DFU device, and whether the framing controls + Connect button render. The DFU tab itself ([DfuFlashView.tsx](src/apps/serial/views/DfuFlashView.tsx)) now reads `dfuSerial` + `dfuDevices` from the serial store instead of carrying its own device list — strips ~40 lines, and the Connect/Echo/Reset top-nav buttons hide automatically when the DFU tab is active. Locale strings under `serialPortPicker.dfu*` (common) and `dfu.*` (serial) added.

- **Serial → STM32 UART flasher (AN3155 system bootloader)**: New fourth tab in the Serial app that mirrors the ESP32 Flash tab for STM32 chips reachable over a UART. Implements ST's AN3155 protocol directly in Rust ([src-tauri/src/flashers/stm32_flasher.rs](src-tauri/src/flashers/stm32_flasher.rs)) — autobaud `0x7F` sync, GET / GET_ID, READ_MEMORY, WRITE_MEMORY (256-byte word-aligned chunks padded with `0xFF`), and EXTENDED_ERASE / legacy ERASE for mass-erase. Three operations behind a segmented control in [src/apps/serial/views/Stm32FlashView.tsx](src/apps/serial/views/Stm32FlashView.tsx) — **Flash** (accepts raw `.bin` at a user-supplied address defaulting to `0x08000000`, or Intel `.hex` parsed via the `ihex` crate with each record carrying its own address), **Backup** (chunked READ_MEMORY loop with progress + cancel; surfaces "Read protected (RDP Level 1)" when the chip rejects the read), and **Erase** (mass-erase, two-click confirm). Bootloader entry is auto-pulsed via RTS/DTR with stm32flash-style defaults (DTR=BOOT0, RTS=NRST active-low) — pin mapping, polarity, and baud are user-configurable per-board, with a `none` setting for hardware that doesn't wire those lines. Chip detection auto-runs on tab entry: a status bar shows the friendly chip name + PID + bootloader version + flash size from a built-in lookup table covering F0/F1/F2/F3/F4/F7/L0/L1/L4/G0/G4/H7/WB/WL families, plus an amber "RDP locked" pill when readout protection is set. Five new Tauri commands (`flasher_stm32_*`) registered in [src-tauri/src/lib.rs](src-tauri/src/lib.rs); shares the existing `flasher-progress` event channel, `register_flash` / `is_cancelled` / `finalise_flash_result` plumbing, and the same port hand-off ref + phase-watch effect in [src/apps/serial/Serial.tsx](src/apps/serial/Serial.tsx) so swapping between Terminal and STM32 UART is non-destructive. Unit tests in `stm32_flasher.rs` cover the Intel HEX parser (contiguous/non-contiguous spans), word-padding helper, PID lookup, and pin parser. New `ihex = "3"` dependency in [src-tauri/Cargo.toml](src-tauri/Cargo.toml).

- **Serial → ESP32 Flash: live wire-up + Backup + Erase modes**: The ESP Flash tab's Rust stubs are replaced with a real `espflash`-driven implementation in [src-tauri/src/flashers/esp_flasher.rs](src-tauri/src/flashers/esp_flasher.rs). Three operations live behind a segmented control in [src/apps/serial/views/EspFlashView.tsx](src/apps/serial/views/EspFlashView.tsx) — **Flash** (write a `.bin` to a given offset, defaulting to `0x0`), **Backup** (dump a region or the whole chip to a file using a chunked ReadFlash loop so the progress bar and Cancel button stay responsive), and **Erase** (full-chip wipe via `flasher.erase_flash()`, with a two-click confirm). Chip detection is the primary path: a status bar at the top of the tab auto-runs detect the moment the user enters the tab with a port selected (and again whenever the port changes), surfacing chip model, MAC, and detected flash size; the Flash size dropdown auto-populates from the result. The form mirrors the user-facing knobs from `esptool ... write-flash` — chip, flash baud (default 921 600 to match the typical wired-flexlink workflow), flash mode (default `dio`), flash freq (default `80MHz`), and flash size — persisted in the serial store as `espOptions` so they survive tab switches. When writing to address `0x0` and the buffer starts with the ESP image magic byte `0xE9`, the bootloader header (bytes 2-3) is patched in-place with the user's mode/freq/size, mirroring esptool's behaviour. Cancellation registry shared with DFU; final phase resolution (Done / Cancelled / Error) centralised in `finalise_flash_result`. New Tauri commands `flasher_esp_read_flash` and `flasher_esp_erase` registered in [src-tauri/src/lib.rs](src-tauri/src/lib.rs); `flasher_esp_detect_chip` and `flasher_esp_flash` extended to take an `EspFlashOptions` payload. The `espflash` crate's `serialport` feature is now enabled in [src-tauri/Cargo.toml](src-tauri/Cargo.toml) (previously off due to `default-features = false`, which gated `Connection`/`Flasher`/`ProgressCallbacks` behind a feature flag).

- **Serial → top-nav Connect / Disconnect button**: Pulled the connect action out of the SerialPortPicker popover and onto the top nav as a first-class button so the user doesn't have to open the picker every time. Pinned as the leftmost action so its position doesn't shift when tabs change ([src/apps/serial/views/SerialTopBar.tsx](src/apps/serial/views/SerialTopBar.tsx)) — green Plug icon when disconnected (disabled until a port is picked), red Unplug when connected, with Echo + Reset rendering after it only while connected.

- **Serial → automatic port hand-off between Terminal and ESP Flash**: Switching from a connected Terminal session to the ESP Flash tab no longer drops the port. The eager auto-detect now skips when the Terminal has the port open; whenever the user explicitly kicks off a Detect / Flash / Backup / Erase, [Serial.tsx](src/apps/serial/Serial.tsx) remembers the Terminal was open, hands the port over, and reopens the Terminal automatically once the ESP phase settles to `done` / `error` / `cancelled` (driven by a phase-watching `useEffect`, with a `restoreTerminalAfterEspOp` ref to gate the restore so a manual disconnect mid-flow isn't fought).

- **Serial app** (utilities group): New UART terminal for talking to microcontrollers (Zephyr shell, MicroPython REPL, AT commands, vendor bootloaders). Layout matches Decoder — top nav with icon + vertical separator + identity picker, viewport bubble below with tabs (Terminal / ESP32 Flash / STM32 DFU). Talks to the port **directly** through new `serial_terminal_*` Tauri commands ([src-tauri/src/serial_terminal.rs](src-tauri/src/serial_terminal.rs)) — no IO sessions / multi-source broker / ad-hoc profile machinery. The Rust side opens the port, splits read/write via `serialport::try_clone()` (reader thread owns its handle outright; writes go through a separate cloned handle behind a mutex), and emits raw bytes on `serial-terminal-data`. Read/write run on independent OS handles backed by the same FD, so write latency isn't bounded by the read timeout — keystroke round-trip feels instant. New shared **port picker** component ([src/components/SerialPortPicker.tsx](src/components/SerialPortPicker.tsx)) modelled on `FrameLinkDevicePicker`: compact button with status dot, click opens a popover listing every detected serial port with the matching `kind: "serial"` IO profile surfaced as a chip; baud / data / stop / parity controls in the popover footer. Right-side actions: **Local Echo** toggle (Eye/EyeOff) and **Reset** (pulses RTS+DTR low → high to reset the connected µC, covers ESP32 EN, Arduino DTR, Black Pill DAPLink). Terminal view ([SerialTerminalView.tsx](src/apps/serial/views/SerialTerminalView.tsx)) embeds [xterm.js](https://xtermjs.org/) (`@xterm/xterm` + `@xterm/addon-fit`) so ANSI escape sequences render as colours and cursor moves work; small inner padding so text doesn't touch the bubble edges. The `useSerialTerminal` hook ([src/apps/serial/hooks/useSerialTerminal.ts](src/apps/serial/hooks/useSerialTerminal.ts)) registers Tauri event listeners with a `cancelled` flag in the cleanup closure — covers React 19 StrictMode's double-mount race where `listen()`'s promise resolves after the first cleanup runs (otherwise the orphan listener stays alive and every event fires twice). Two further tabs (**ESP32 Flash** and **STM32 DFU**) ship with their full UI (file picker, chip detect, progress bar, log pane, cancel) and a backend command surface (`flasher_esp_*` / `flasher_dfu_*`) emitting on the `flasher-progress` event channel — the actual flash loops wrap `espflash` and `dfu-nusb` (added to `src-tauri/Cargo.toml`) and currently return a clear "not yet wired up" error pending hardware verification. Flashers are gated to desktop builds (iOS excluded).

- **App group reshuffle**: Test Pattern and Sessions moved from utilities → sessions (alongside Discovery / Decoder / Transmit / Graph / Modbus); Serial sits in utilities (alongside Catalog Editor / Calculator). Reflects how each app is actually used — Test Pattern and the Sessions panel are session-bound, Serial is a self-contained µC tool.

### Changed

- **Serial → unified Flash tab + driver registry**: The three per-chip Flash tabs (ESP32 / STM32 UART / STM32 DFU) collapsed into a single **Flash** tab driven by a `FlasherDriver` registry ([src/apps/serial/flashers/registry.ts](src/apps/serial/flashers/registry.ts)) — adding a new chip family is one new driver record, no changes to the view, store, or hand-off lifecycle. Each driver ([espDriver.tsx](src/apps/serial/flashers/espDriver.tsx), [stm32Driver.tsx](src/apps/serial/flashers/stm32Driver.tsx), [stm32DfuDriver.tsx](src/apps/serial/flashers/stm32DfuDriver.tsx)) is a plain record carrying its `OptionsPanel`, capability flags (flash/backup/erase), `flash`/`backup`/`erase` callbacks, image extensions, default flash address, and a `describeChip` function. A new Tauri command `flasher_serial_detect` ([src-tauri/src/flashers/detect.rs](src-tauri/src/flashers/detect.rs)) probes a port for STM32 AN3155 first (single 0x7F handshake, fast) then ESP esptool, returning a `DetectedChip` payload with a `driver_id` that picks the active driver. DFU detection is implicit — the manufacturer string from the USB enumeration record (`"STM32 DFU"` for the ST ROM bootloader, generic `"DFU"` otherwise) maps to a driver via [registry.ts](src/apps/serial/flashers/registry.ts). The unified [FlashView.tsx](src/apps/serial/views/FlashView.tsx) reads the active driver and renders the entire UI off it: chip status bar with a coloured `ManufacturerBadge` ([src/components/ManufacturerBadge.tsx](src/components/ManufacturerBadge.tsx)) — cyan for ESP32, amber for ESP8266, sky for STM32 (UART), violet for STM32 DFU — plus the driver's `describeChip` line; mode toggle gated by capabilities (DFU disables Backup + Erase); driver-specific options panel; common operation form (image picker, address, output path, full-chip toggle). A new [flasherStore.ts](src/apps/serial/stores/flasherStore.ts) carries the active driver id, single shared progress channel (only one op runs at a time across all chips), detected chip, and per-driver option slices; the [serialStore](src/apps/serial/stores/serialStore.ts) shrinks to terminal + DFU enumeration + a `flashTarget` enum that switches the picker between port and DFU. The auto-detect logic moved into a new [useChipDetection](src/apps/serial/flashers/useChipDetection.ts) hook keyed on `serial:<port>` / `dfu:<usb-serial>` so re-runs only fire when the device actually changes. Three per-chip view files (`EspFlashView.tsx`, `Stm32FlashView.tsx`, `DfuFlashView.tsx`) deleted; the per-tab phase-watch hand-off effect collapsed from two copies (ESP + STM32) into one. Locale strings under `tabs.espFlash` / `tabs.dfuFlash` / `tabs.stm32Flash` and the entire `dfu.*` section are gone — replaced with a single `tabs.flash` key.

- **Settings → Devices: unified tabbed device page**: The wizard's linear step machine (CredentialsView → ProvisioningView → ProvisionCompleteView → InspectView → UploadView → UpgradeCompleteView, plus FrameLinkSetupView) was funnelling the user down a single path determined by which button they clicked first; back buttons were re-implemented per view (some with arrows, some without, some labelled "Back", some "Disconnect", some targeting `scan`, others `inspect`) and IP firmware upgrade was unreachable when a device advertised both FrameLink and SMP because the IP-button branch always preferred FrameLink. Replaced with a single device page ([src/apps/devices/components/DeviceView.tsx](src/apps/devices/components/DeviceView.tsx)) hosting one consistent header ([DeviceHeader.tsx](src/apps/devices/components/DeviceHeader.tsx)) — single back button that drops every live transport before returning to scan — and four tabs (Wi-Fi (BLE) / Firmware (BLE) / Firmware (IP) / Data IO). Tabs the device doesn't expose stay clickable but render an explainer panel with shortcuts to the available alternatives. Discovery routes deterministically into the right initial tab: BLE → Wi-Fi (BLE) for wifi-provision-capable devices, IP → Firmware (IP) for SMP-capable devices, IP → Data IO for FrameLink-only devices, manual IP routes by chosen protocol. Per-transport connect/disconnect plumbing centralised into [useDeviceConnection.ts](src/apps/devices/hooks/useDeviceConnection.ts) with a module-level in-flight Promise cache that survives React 19 StrictMode's double-effect (previously two `bleConnect()` calls fired in the same millisecond and panicked btleplug). Tabs themselves never disconnect, navigate, or reach into other tabs' state. Firmware (BLE) is currently marked unavailable with an explainer message because `mcumgr-smp` still pins btleplug 0.11 — a `BLE_SMP_DISABLED` constant in `DeviceView.tsx` flips it back on once the dependency lands.

- **Settings → Devices: one card per physical device**: A single device used to render as up to three separate cards (BLE, mDNS FrameLink, mDNS SMP) because the store keyed each entry by transport-prefixed ID. New `MergedDevice` selector ([src/apps/devices/utils/mergedDevices.ts](src/apps/devices/utils/mergedDevices.ts)) groups by name and feeds a redesigned [src/components/MergedDeviceCard.tsx](src/components/MergedDeviceCard.tsx) with four capability badges (BLE/WiFi/SMP/FrameLink) using a two-layer encoding — **edge ring (always blue) lights when BLE advertises the capability**, **centre fills in the capability's own colour when mDNS confirms it**. Metadata bubbles ringed per protocol: `rssi` blue, `addr` purple, `smp_port` amber, `port` (FrameLink) green. Two connect buttons per card (Connect via BLE in blue, Connect via IP in purple) light up only when the relevant transport is present, with translatable `aria-label`s for screen readers. BLE devices initially seen as MAC fold into the named bubble once the name resolves on a later poll. WiFi badge uses the same purple as the IP button (semantic: WiFi → IP connectivity). All visible strings go through `t()` under a new `card.*` namespace in [src/locales/en-AU/devices.json](src/locales/en-AU/devices.json).

- **Centralised app registration in [src/apps/apps.json](src/apps/apps.json) + [src/apps/registry.ts](src/apps/registry.ts)**: Replaced per-surface duplication with a single source of truth. Structural data (id, label, group, accelerator, singleton) lives in `apps.json`; visual concerns (icon, colour, lazy import) layer on in `registry.ts`. [src/components/LogoMenu.tsx](src/components/LogoMenu.tsx), the Watermark dashboard, the `AppTab` component, the Dockview component registry, and the native Tauri Apps menu all derive from these two places. The Rust menu reads `apps.json` via `include_str!` and builds the Apps submenu with separators between groups, matching the LogoMenu order (sessions / database / framelink / utilities). Menu-event dispatch is data-driven via the app-prefix rather than a hand-kept match arm. The Watermark dashboard renders groups inline with vertical dividers, mirroring the LogoMenu structure. [docs/style_guide.md](docs/style_guide.md) "Adding a new app" rewritten for the two-file flow.

- **Rules app aligned with shared app conventions**: Extracted `RulesTopBar` wrapper following the Discovery/Decoder pattern; dropped the inline `<select>` and the redundant "Rules" title next to the icon. Added shared [FrameLinkDevicePicker](src/components/FrameLinkDevicePicker.tsx) (button + popover, status dot per device) and `useFrameLinkDeviceLiveness` hook (mDNS scan + lazy probe) for use across FrameLink-aware apps. Replaced the bespoke main view with the data-view bubble + `dataViewTabClass` tabs to match Decoder/Discovery spacing. Switched error styling from raw `bg-red-500/10` to `bgDanger` / `borderDanger` / `textDanger` so it themes correctly. Dropped the operational footer in favour of an in-memory **Log tab** (capped at 200 entries) that mirrors every status update with the entity ID included in each message ("Bridge 0x0001 added"). Style guide updated to document the top-bar layout, identity-picker pattern, error surfaces, main-view bubble, and activity-log conventions.

- **Settings → Data IO: FrameLink config grouped per interface**: Replaced the two-section Interfaces + Device Configuration layout with a single Interfaces list where each entry is a collapsible card. Header shows the bitrate (muted) and the protocol type as a chip; expanding the card reveals only that interface's signals, grouped by signal group. Added `tlog!` / `tlog.debug` instrumentation in `framelink_get_interface_signals` and the `loadFlSignals` loader so the device-advertised signal IDs are visible at debug level on both sides for cross-checking.

### Fixed

- **Stale BLE devices linger after power-off**: The BLE scanner held cached peripherals from the OS, so a powered-off device kept appearing in Settings → Devices until the next manual rescan. The store now stamps `lastSeenAt` on every emit and prunes entries older than 4 s while a scan is active. The Rust BLE poll loop drops the change-only suppression so every visible peripheral re-emits every ~500 ms (verbose logging is still gated to changed state). The mDNS receiver caches resolved services and re-emits them every poll as a heartbeat, and honours `ServiceRemoved` events — fixes a related regression where mDNS-discovered cards (FrameLink/SMP) vanished after the 4 s prune cutoff because mDNS only fires `ServiceResolved` once.

- **Settings → Devices count mismatch**: "Found N devices" header used the raw per-transport store length rather than the merged card count, reporting e.g. "Found 4 devices" when only 2 cards rendered. Header and empty-state spinner check now both use `mergedDevices.length`.

- **FrameLink devices missing from unified scan**: `device_scan.rs`'s BLE path silently dropped any peripheral without a `local_name` and only matched the WiFi-prov + SMP service UUIDs, so a FrameLink device in normal operating mode (advertising the FrameLink primary service) never reached the UI. Now uses `parse_peripheral` from the patched `framelink-rs` to match by primary service + manufacturer-data bitmask, falling back through `advertisement_name` / `local_name` / `ble_id` for naming. UUID-only matches retained as fallbacks for legacy non-FrameLink devices. mDNS-discovered devices now also surface the wifi-provision capability chip — reaching the host via mDNS implies the device is already on the network.

- **Test Pattern missing from native Apps menu, Cmd+9 opening nothing**: Test Pattern was absent from the native Tauri Apps menu (only present in the LogoMenu dropdown). The Cmd+9 accelerator was bound to `app-calculator` but the Frame Calculator panel registered under `app-frame-calculator`, so the shortcut did nothing. Both fixed as part of the centralised app registration sweep above.

### Added

- **Full localisation rollout**: Wired `react-i18next` into the app with English (Australian) as the source/fallback locale. Translation files live under [src/locales/en-AU/](src/locales/en-AU/) split by namespace, with the registry in [src/locales/index.ts](src/locales/index.ts) and bootstrap in [src/i18n.ts](src/i18n.ts) imported once from [src/main.tsx](src/main.tsx). New `language` setting threaded through Rust (`AppSettings`), TypeScript (`useSettings`), the settings store, and a Language picker in `GeneralView` — `WireTAP.tsx` calls `i18n.changeLanguage()` whenever the setting changes. Adding a new locale only requires dropping a folder alongside `en-AU/` and registering it. Every shell, view, dialog, panel, sub-panel, and shared component across all apps now flows through `useTranslation` — including the ~1700-line `IOProfileDialog`, the full IoSourcePicker / CsvColumnMapper dialog trees, and shared playback/session/picker components. Pluralisation uses i18next `_one`/`_other` variants; date and number formatting goes through `Intl` with `i18n.language` so locale switches take effect without restart. The remaining inline literals are stragglers in helper components and can be cleaned up as the surrounding code is touched.
...
- **StreamEnded WS decoder mismatch**: The frontend `decodeStreamEnded` was reading length-prefixed strings (an older format), but the Rust encoder has always used a compact binary format (reason byte, flags byte, conditional fields). This caused a `RangeError` when leaving sessions. Rewrote the decoder to match the Rust wire format.

- **`TemporalMode::Buffer` → `Capture`**: Renamed the third temporal mode from `Buffer` to `Capture` across the full stack. Rust enum `TemporalMode::Buffer` → `Capture`, `CaptureSource::source_type()` returns `"capture"` instead of `"buffer"`, traits match arm updated. **Breaking wire change**: serialised `temporal_mode` field changes from `"buffer"` to `"capture"`. Frontend `TemporalMode` type, all `=== "buffer"` comparisons, and `sourceType === "buffer"` checks updated.

- **Remaining naming remnants sweep**: Cleaned up leftover old names from the Buffer→Capture, Listener→Subscriber, and Reader→Source renames. TypeScript: `SessionSwitchedToBufferPayload` → `SessionSwitchedToCapturePayload`, `transitionToBufferReader()` → `transitionToCaptureSource()`, `switchSessionToBufferReplay()` → `switchSessionToCaptureReplay()`, `useEffectiveBufferMetadata()` → `useEffectiveCaptureMetadata()`, `useBufferFrameView()` → `useCaptureFrameView()`, `onSwitchedToBuffer` → `onSwitchedToCapture`, `LocalSessionState.buffer` → `.capture`, payload fields `buffer_id`/`buffer_count`/`buffer_type` → `capture_id`/`capture_count`/`capture_kind`. Rust: `ListenerEvictedPayload` → `SubscriberEvictedPayload`, `transition: "buffer"` → `"capture"`, ~60 comment/tlog "listener" → "subscriber" updates. Documentation: session-flow.md, capture-flow.md, capture-database-schema.md, CLAUDE.md updated.

- **Last-mile buffer→capture vocabulary cleanup**: Renamed all remaining internal variable names, prop names, function names, parameter names, and comments that said "buffer" when meaning "capture". Key renames: `bufferMode` → `captureMode`, `enableBufferMode` → `enableCaptureMode`, `useBufferFirstMode` → `useCaptureFirstMode`, `onToggleBufferPin`/`onRenameBuffer`/`onClearBuffer` → `onToggleCapturePin`/`onRenameCapture`/`onClearCapture`, `handleClearBuffer` → `handleClearCapture`, `resumeWithNewBuffer` → `resumeWithNewCapture`, `switchToBufferReplay` → `switchToCaptureReplay`, IoSourcePickerDialog internals (`buffers` → `captures`, `selectedBuffer` → `selectedCapture`, etc.), CaptureList/SourceList/ActionButtons props, API functions (`useBuffer` → `useCapture`, `stepBufferFrame` → `stepCaptureFrame`, `copyBufferForDetach` → `copyCaptureForDetach`), settings (`clearBuffersOnStart` → `clearCapturesOnStart`, `discoveryHistoryBuffer` → `discoveryHistorySize`), Query app state, and ~150 comments across stores, hooks, dialogs, and API wrappers. Genuine ring buffers (`_frameBuffer`, `serialBytesBuffer`, `seriesBuffers`, `maxBuffer`) deliberately left unchanged.
...
- **`temporal_mode: "timeline"` → `"recorded"`**: The `TemporalMode::Timeline` enum variant is now `Recorded`, matching the UI's existing "Recorded Sources" label. Rust module `io/timeline/` renamed to `io/recorded/`. `timeline_can()` → `recorded_can()`, `TimelineControl` → `PlaybackControl`. **Breaking wire change**: the serialised `temporal_mode` field changes from `"timeline"` to `"recorded"` on `InterfaceTraits`. Frontend `TemporalMode` type and all comparisons updated in lockstep.
...
- **Buffer → Capture rename (Rust internals)**: Complete the internal rename across 26 Rust backend files — struct fields (`buffers` → `captures`, `orphaned_buffer_ids` → `orphaned_capture_ids`), function names (`emit_buffer_changed` → `emit_capture_changed`, `stop_and_switch_to_buffer` → `stop_and_switch_to_capture`, `send_buffer_changed` → `send_capture_changed`, etc.), local variables, parameters, WS protocol enum (`MsgType::BufferChanged` → `MsgType::CaptureChanged`, wire byte `0x07` unchanged), settings fields (`clear_buffers_on_start` → `clear_captures_on_start` with serde alias for backward compatibility), log prefixes, error messages, and comments. Wire protocol string literals (`temporal_mode === "buffer"`, `device_type: "buffer"`) preserved for lockstep frontend rename.
...
- **CI: pre-create draft release before build matrix**: The GitHub Actions build workflow now creates the draft GitHub release in a dedicated job before the build matrix runs. This avoids the race condition where multiple parallel `tauri-action` invocations each tried to create the release simultaneously, causing all but one to fail with a 422 conflict.
...
- **Decoder (and other `onFrames` listeners) saw no frames during capture playback**: `CaptureSource`'s stream loop built frames into `batch_buffer`, then cleared it without sending, and relied on `signal_frames_ready → send_new_frames`. That dispatch path reads newly-appended rows from the capture store — fine for live sources, but capture replays read existing rows without appending, so `total == offset` forever and no `FrameData` was ever broadcast over WebSocket. Discovery masked the bug because it pages SQLite directly via `useBufferFrameView` driven by playback-position updates; Decoder relies entirely on the `onFrames` WS callback and saw nothing. `CaptureSource` now calls `crate::ws::dispatch::send_frames` at every batch flush (no-pacing, high-speed pacing, normal-speed pacing, final remaining batch) and sends the single frame directly in the normal-speed single-frame path. `CaptureSource::new` also calls `capture_store::set_capture_owner` so the session's capture is discoverable to lookups (matching every other reader).
...
- **Decoder session button showed "Capture" after leaving a live session**: After leaving a live capture, Decoder's SessionButton rendered the literal string `"Capture"` (the fallback at `displayName = captureMetadata?.name || captureMetadata?.id || "Capture"`) because `captureMetadata` was still null — the manager's automatic live→capture transition sets `ioProfile=captureId` but Decoder had no `onSessionDestroyed` handler to fetch the new capture's metadata. Discovery already has this handler. Added the mirror in Decoder so the label, tooltip (id, time range, frame counts), and timeline populate immediately on leave without waiting for the user to click Play.

- **Leaving a live capture left the resulting orphaned capture unplayable**: After leaving a live multi-source session, the frontend re-set the session to the orphaned capture id, but calling Play produced `No profile IDs or source configs found for session ... Cannot resume to live`. Root cause was a transport race in `useIOSession`: `session-lifecycle destroyed` arrives via a Tauri event and triggers `cleanupDestroyedSession`, which unsubscribes the session's WebSocket channel *before* the still-in-flight `StreamEnded` WS message is processed. `StreamEnded` was the only path that added the capture id to `knownCaptureIds`, so the post-destroy re-setup found `isCaptureProfileId(captureId) === false`, routed through `create_reader_session` instead of `create_capture_source_session`, and the resulting session had no profile or source configs. The destroy and listener-evicted handlers in `useIOSession` now call `addKnownCaptureId` for each orphaned/copied capture id they receive — this is the authoritative "this capture now exists as an orphan" signal and doesn't depend on WS message ordering.

- **Data source picker collapsed-view flicker during a running capture**: Clicking "Change" on a collapsed source entry briefly expanded the full picker and then snapped back to collapsed while the capture was streaming (but not while paused). The big init `useEffect` had `[isOpen, loadProfileId, selectedId, selectedIds]` as its deps and reset `hasUserExpandedRef` to `false` on every run. During streaming, Discovery re-renders frequently and `selectedIds` (derived from `ioProfiles`) can change reference without a real value change, re-running the init effect and clobbering the "user expanded" flag. The next 2-second `activeMultiSourceSessions` poll then re-collapsed the view. Added a `didInitForOpenRef` guard so the init effect runs exactly once per open cycle and narrowed the dep array to `[isOpen]`.

- **Session tooltip always showed `Frames: 0` for capture-backed sessions**: two separate paths left `session.capture.count` stuck at `0` while the backend knew the real count (the IO picker, which queries `list_active_sessions` directly, showed it correctly). (1) `openSession` in `sessionStore` hardcoded `count: 0`/`available: false` when creating a capture-backed session and the async metadata fetch only copied `name`/`persistent` — now it also copies `count`, `available`, `kind`, `start_time_us`, `end_time_us`. (2) The `SessionLifecycle` handler's live→buffer transition branch (triggered by `stopAndSwitchToBuffer`) relied on `prevSession.capture.count` being populated by a prior `StreamEnded` message, which could be absent or wiped by an intermediate `running` lifecycle blip that resets `capture` to zeros. It now fetches fresh metadata via `getCaptureMetadataById` and updates the session's capture fields, matching the pattern in `openSession`.

- **CSV import left Discovery's tooltip/frame picker/Tools button stuck at 0**: After importing a CSV in the data source picker, the frame table paged the new capture correctly but the session hover tooltip showed `Frames: 0, Unique: 0`, the frame-ID picker showed `0/0`, and the `Tools` button stayed disabled. `handleCsvMapperComplete` called `onSelect(metadata.id)` to route the new capture back through Discovery's `handleIoProfileChange`, which branches on `isCaptureProfileId(id)` — but `knownCaptureIds` in `sessionStore` is only populated at app start and pruned on delete, never added to on create. The guard returned `false`, the non-buffer branch ran, `watchSource`/`switchToCapture` were skipped, and the `onAfterSwitch` callback that calls `getCaptureFrameInfo` → `setFrameInfoFromBuffer` never fired. Now `handleCsvMapperComplete` calls `addKnownCaptureId(metadata.id)` before `onSelect`, and the cross-window `BUFFER_CHANGED` listener in `sessionStore` also registers `event.payload.metadata.id` so any other window that observes the event stays consistent. Also renamed the `Buffers` heading in the data source picker to `Captures` (missed by the earlier buffer→capture rename sweep).
...
- **Split view tab space**: Collapsed the empty prefix-actions container in secondary split panes — tabs now reclaim the space reserved for the logo menu when it isn't rendered
- **Empty state styling**: Standardised all no-data / placeholder text across the app to use the `emptyState` typography tokens — consistent font, size, colour, and centering in ~20 components (Decoder, Discovery, Graph, Catalog, Settings, Transmit, dialogs)
...
- **Rebrand: CANdor → WireTAP**: The project has been renamed from CANdor to WireTAP across the entire codebase. The splash screen and About dialog reference "the artist formerly known as CANdor". Specific changes:
  - Tauri bundle identifier → `com.wiredsquare.wiretap`, binary → `WireTAP`
  - Rust crate → `wiretap` / `wiretap_lib`
  - Data directory auto-migrates from `~/Documents/CANdor/` → `~/Documents/WireTAP/` on first launch; persisted paths in settings.json are also patched
  - Default config path → `config/wiretap.toml`
  - Log files → `*-WireTAP.log`
  - MQTT defaults → `wiretap/#` topics, `wiretap-` client ID prefix
  - Default PostgreSQL database name → `wiretap`
  - Reports footer → "Generated by WireTAP"
  - `tools/candor-server/` → `tools/wiretap-server/` (including systemd unit, config, and script)
  - GitHub URLs → `Wired-Square/WireTAP`
  - Keyring service name intentionally kept as `com.candor.io-profiles` to preserve existing stored credentials

### Added

- **Modbus TCP multi-source support**: Modbus TCP is now a first-class multi-source device, using the same `MultiSourceReader` architecture as CAN adapters. Sessions get `m_` prefixes (instead of the incorrect `t_` timeline prefix) and carry proper `InterfaceTraits` with `Protocol::Modbus`. Supports combining multiple Modbus TCP connections in a single session with interface mapping (each device maps to a distinct output bus). Each interface declares a role — **client** (polls a device) or **server** (accepts incoming connections for MITM observation). Shared poll groups from the catalog are injected into all Modbus interfaces in the session.
...
- **Sungrow SHx decoder catalog**: Bundled example catalog for Sungrow SH-series hybrid solar inverters (SH5K–SH25T) via Modbus TCP. Covers ~90 signals across 42 poll groups: version strings, protocol version, ARM/DSP firmware, device info, all four MPPTs, per-string PV data (up to 5 strings), grid voltages/currents/frequency, reactive power, power factor, insulation resistance, battery power/voltage/SoC/SoH/temperature, per-phase meter power/voltage/current/frequency, backup power/current/voltage/frequency, PV/import/export/charge/discharge energy totals, system state, and holding register readback (EMS mode, forced charge/discharge, SoC limits, export limits, power gradients, active power limitation, per-phase export mode, site import limit, battery power thresholds).
...
- **Session Cleanup on External Destroy**: Fixed apps (Discovery, Decoder, etc.) showing stale session UI after a session is destroyed from the Sessions app. Apps now listen for `session-lifecycle` and `buffer-orphaned` events and automatically transition to buffer mode with the orphaned buffer. Rust backend reordered to emit buffer-orphaned before session-lifecycle, and added missing buffer orphaning in the auto-destroy path when the last listener leaves.

- **Leave While Streaming**: Fixed clicking "Leave" in an app while streaming showing a stale session ID with a grey dot requiring a second click. Now suspends the session first (creating a buffer), copies it, and transitions to buffer mode in a single action.
