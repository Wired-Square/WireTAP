...
- **The "Graph" app is now "Dashboard", with an analog-gauge icon**: the app id, label, native-menu entry, launcher/top-bar icon (now lucide `Gauge`), and the "send to" actions in Decoder/Discovery were renamed from graph → dashboard. Internal type names (`graphStore`, `GraphPanel`), the `graph/` source directory, the i18n namespace, and the `graph.layouts`/`graph.lastSession` storage keys are unchanged to avoid migration; a legacy `graph` Dockview component alias keeps previously-saved window layouts loading. [src/apps/apps.json](src/apps/apps.json), [src/apps/registry.ts](src/apps/registry.ts), [src/components/MainLayout.tsx](src/components/MainLayout.tsx).

### Added

- **Catalog `display` hints + "Add as Instruments"**: a signal can carry a `display` hint in its catalog (shorthand `display = "rotary"`, or a table `display = { widget = "level-bar", orientation = "vertical" }`); the Rust crate ignores the unknown key, so the frontend recovers it by re-parsing the catalog's raw TOML. A new wand-icon top-bar action opens the signal picker in a standalone "Add as Instruments" mode that creates one pre-configured widget panel per selected signal — using the explicit hint, else inferring from the signal's metadata (enum → indicator, bounded + angular unit → rotary, bounded → level bar, otherwise line chart). `min`/`max` are now surfaced from the catalog crate model. [src/apps/graph/widgets/displayHints.ts](src/apps/graph/widgets/displayHints.ts), [src/apps/graph/widgets/autoWidget.ts](src/apps/graph/widgets/autoWidget.ts), [src/apps/graph/dialogs/SignalPickerDialog.tsx](src/apps/graph/dialogs/SignalPickerDialog.tsx).

- **Visualisation widget registry + six new instrument primitives in the Graph app**: the Graph app's panel types are now a registry (`widgetMeta.ts` + `registry.tsx`) rather than a hardcoded conditional, so the add-panel menu, rendering, image-export surface routing and per-type defaults all derive from one table. Six widgets join the existing line-chart/gauge/list/flow/heatmap/histogram: **indicator** (icon-state — maps an enum value to a lucide glyph + colour + brightness), **rotary / steering-wheel** (a bounded scalar → rotation), **level bar + readout**, **bitfield** (per-bit on/off of a frame's latest bytes), plus two scriptable surfaces (below). The widget config dialog gained range fields for rotary/level-bar, a frame-id picker for bitfield, and a code editor for the custom widgets. [src/apps/graph/widgets/](src/apps/graph/widgets/), [src/apps/graph/views/GraphGrid.tsx](src/apps/graph/views/GraphGrid.tsx), [src/stores/graphStore.ts](src/stores/graphStore.ts).

- **Animated, scriptable custom-canvas and custom-SVG widgets, sandboxed in a Web Worker**: a custom widget runs author-supplied draw code — `(ctx, { signals, width, height, time, dt })` for canvas, or `(signals, { … }) → svg markup` for SVG — driven by a main-thread requestAnimationFrame loop so it animates independent of data rate. The code runs in a dedicated Worker with no DOM, no Tauri IPC and network/storage globals scrubbed; canvas frames return an `ImageBitmap` (so PNG export still works), returned SVG markup is DOMPurify-sanitised before injection, and a host watchdog terminates code that misses its per-frame deadline. Custom-SVG also has a declarative scene-graph mode (composed gauges/needles/lamps/bars/text) that executes no code at all. The author-code path requires `'unsafe-eval'` in the app CSP — the Worker sandbox remains the real boundary. [src/apps/graph/widgets/customWorker.ts](src/apps/graph/widgets/customWorker.ts), [src/apps/graph/widgets/customWorkerHost.ts](src/apps/graph/widgets/customWorkerHost.ts), [src-tauri/tauri.conf.json](src-tauri/tauri.conf.json).

- **Standalone dashboard artifacts (`*.dashboard.json`)**: panels + layout can be saved as a shareable file in a `dashboards/` subdirectory of the decoder directory and reopened from the Graph → Layouts menu, alongside the existing in-store named layouts. New Rust list/open/save commands and a frontend schema (`wiretap.dashboard/1`). [src-tauri/src/dashboard.rs](src-tauri/src/dashboard.rs), [src/utils/dashboards.ts](src/utils/dashboards.ts), [src/api/dashboards.ts](src/api/dashboards.ts).
