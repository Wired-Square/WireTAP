...
- **Agent/MCP-created sessions are first-class in the UI**: backend sessions (including those an MCP agent opens) are adopted into the session store as *known-only* (connected + capabilities, no frame subscription), so an agent-started repeat's Transmit row is fully controllable (start/stop/restart) and session-aware panels can show its source. Adds the `attach_source(session_id, panel)` MCP tool, which opens/focuses a source-aware tab (discovery, decoder, transmit, query, or graph) and points it at a session. [src/hooks/useSessionRosterSync.ts](src/hooks/useSessionRosterSync.ts), [src/stores/sessionRoster.ts](src/stores/sessionRoster.ts), [src/stores/transmitRowSession.ts](src/stores/transmitRowSession.ts), [src-tauri/src/mcp/tools.rs](src-tauri/src/mcp/tools.rs).
