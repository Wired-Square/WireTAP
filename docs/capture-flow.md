...
the frontend, from the manager's `sourceProfileId` — the session's
`originProfileIds` in `sessionStore`, Rust's `origin_profile_ids`, so a window
that only joined the session resolves the same owner as the one that started
it, and a stopped source replaying its capture keeps its owner. Every mutation emits `capture-events-changed` with the owner so
